...
/**
 * This class implements a simple energy minimization routine for the
 * side-chains only. Handles the computation of the Amber dihedral energy
 * penalties (for minimizing away from the initial rotamer dihedrals).
 * Additionally there is a special residue, the ligand, that can translate and
 * globally rotate.
 */
public class SimpleMinimizer implements Serializable {
    ...
    // private int MAX_NUM_ATOMS_DISTAL = 30; // the maximum number of atoms
    // distal to the dihedral; this is increased as needed for big ligands
    ...
    /*
     * int sDihedPN[] = new int[0]; double sDihedWeight[] = new double[0]; int
     * sDihedLocalNum[] = new int[0]; int sDihedNumTerms = 0;
     */
    ...
    /*
     * int lDihedPN[] = new int[0]; double lDihedWeight[] = new double[0]; int
     * lDihedLocalNum[] = new int[0]; int lDihedNumTerms = 0;
     */
	...
	// ligResNum = -1;
	/*
	 * for(int i=0;i<3;i++){ ligCurCOM[i] = 0.0; ligStartCOM[i] = 0.0; }
	 */
	...
	for (int i = 0; i < numberOfStrands; i++) {
		...
		if (m.strand[i].residue[j].flexible)
		    MAX_NUM_ATOMS_DISTAL = Math.max(MAX_NUM_ATOMS_DISTAL,
			    m.strand[i].residue[j].numberOfAtoms);
	    }
	}
	...
	for (int i = 0; i < numberOfStrands; i++) {
	    for (int j = 0; j < m.strand[i].numberOfResidues; j++) {
		if (m.strand[i].residue[j].flexible)
		    numStrDihedrals[i] += strandRot[i].rl
			    .getNumDihedrals(curAANum[m.strand[i].residue[j].moleculeResidueNumber]);
	    }
	}
	...
	/*
	 * sysDihedralAtNums = new int[numSysDihedrals][4]; sysDihedralDistal =
	 * new int[numSysDihedrals][MAX_NUM_ATOMS_DISTAL]; sysNumAtomsDistal =
	 * new int[numSysDihedrals];
	 */
	...
	// The first ligand term includes nonbonded terms for computing
	// dihedrals
	...
	for (int str = 0; str < numberOfStrands; str++) {
		...
		if (localRes.flexible) {
		    ...
		    for (int j = 0; j < strandRot[str].rl
			    .getNumDihedrals(curAANum[theM.strand[str].residue[i].moleculeResidueNumber]); j++) {
			...
			strandRot[str].getAtomsMoreDistal(m,
				localRes.moleculeResidueNumber, m.atom[at2num],
				tmpAtomList);
			...
			if (j == 0) {
			    // If this is the first dihedral for this residue
			    // snag information
     ...
     * Amber96ext theA96ff, StrandRotamers theSysRH, StrandRotamers theLigRH,
     * int curAANum[], int curLigAANum, boolean doDihedral, RotamerLibrary rl,
     * RotamerLibrary grl){
     * 
     * MAX_NUM_ATOMS_DISTAL = Math.max(MAX_NUM_ATOMS_DISTAL,
     * theM.strand[theLigStrNum].residue[0].numberOfAtoms);
     * 
     ...
     * doDihedEnergy = doDihedral;
     * 
     * ligRH = theLigRH; ligStrNum = theLigStrNum;
     * 
     * // Now compute the ligand dihedrals // First determine how many there are
     * numLigDihedrals = grl.getNumDihedrals(curLigAANum);
     * 
     * ligDihedralAtNums = new int[numLigDihedrals][4]; ligDihedralDistal = new
     * int[numLigDihedrals][MAX_NUM_ATOMS_DISTAL]; ligNumAtomsDistal = new
     * int[numLigDihedrals];
     * 
     ...
     * 
     * // Now determine which atoms are involved with each dihedral int atoms[]
     * = new int[4]; int tmpAtomList[] = new int[MAX_NUM_ATOMS_DISTAL]; int
     * at3num = -1, at2num = -1; int numDihed = 0; Residue localRes =
     * m.strand[ligStrNum].residue[0]; // Snag all ligand atom information for
     * flexResAtomList, flexResListSize ligResNum = numFlexRes; // numFlexRes
     * was set in the other initialize call int ligResNumPP = ligResNum + 1;
     * flexResListSize[ligResNumPP] = localRes.numberOfAtoms; for(int
     * k=0;k<flexResListSize[ligResNumPP];k++){ flexResAtomList[ligResNumPP][k]
     * = localRes.atom[k].moleculeAtomNumber; }
     * 
     * if(m.strand[ligStrNum].residue[0].flexible){ for(int
     * j=0;j<grl.getNumDihedrals(curLigAANum);j++){ //
     * ligDihedToResNum[numDihed] = 0; // all ligand dihedrals are from residue
     * 0 atoms = grl.getDihedralInfo(m,ligStrNum,0,j); // note: atoms are
     * residue relative numbering ligDihedralAtNums[numDihed][0] =
     * localRes.atom[atoms[0]].moleculeAtomNumber;
     * ligDihedralAtNums[numDihed][1] =
     * localRes.atom[atoms[1]].moleculeAtomNumber;
     * ligDihedralAtNums[numDihed][2] =
     * localRes.atom[atoms[2]].moleculeAtomNumber;
     * ligDihedralAtNums[numDihed][3] =
     * localRes.atom[atoms[3]].moleculeAtomNumber;
     * 
     * at2num = localRes.atom[atoms[2]].moleculeAtomNumber; at3num =
     * localRes.atom[atoms[3]].moleculeAtomNumber;
     * 
     * for(int k=0;k<MAX_NUM_ATOMS_DISTAL;k++){ tmpAtomList[k]=1; }
     * tmpAtomList[atoms[1]]=0; tmpAtomList[atoms[2]]=0;
     * ligRH.getAtomsMoreDistal
     * (m,localRes.moleculeResidueNumber,m.atom[at2num],tmpAtomList);
     * ligNumAtomsDistal[numDihed]=0; for(int k=0;k<MAX_NUM_ATOMS_DISTAL;k++){
     * if ((tmpAtomList[k]==2) && (k != atoms[3])){
     * ligDihedralDistal[numDihed][ligNumAtomsDistal
     * [numDihed]]=localRes.atom[k].moleculeAtomNumber;
     * ligNumAtomsDistal[numDihed] += 1; } } if (j==0){ // If this is the first
     * dihedral for this residue snag information flexResListSize[ligResNum] =
     * ligNumAtomsDistal[numDihed]+1; flexResAtomList[ligResNum][0] = at3num;
	...
	for (int str = 0; str < numberOfStrands; str++) {
		...
		for (int q = 0; q < m.connected[atom2][0]; q++) {
			...
			for (int w = 0; w < m.connected[atom3][0]; w++) {
				...
				// At this point 'q',atom2,atom3,'w' is a
				// dihedral
				if (!(a96ff.getTorsionParameters(
					m.atom[m.connected[atom2][q + 1]].type,
					m.atom[atom2].type, m.atom[atom3].type,
					m.atom[m.connected[atom3][w + 1]].type,
					fConst, eqAngle, terms, multiplic))) {
				    System.out
					    .println("WARNING: Could not find torsion parameters for "
						    + m.connected[atom2][q + 1]
						    + ","
						    + atom2
						    + ","
						    + atom3
						    + ","
						    + m.connected[atom3][w + 1]);
				    return false;
				}
				for (int r = 0; r <= multiplic[0]; r++) {
					...
					System.arraycopy(strDihedPN[str], 0,
						newDihedPN, 0, tmpStrSize);
					System.arraycopy(strDihedWeight[str],
						0, newDihedWeight, 0,
						tmpStrSize);
					System.arraycopy(strDihedLocalNum[str],
						0, newDihedLocalNum, 0,
						tmpStrSize);
	    ...
	    System.arraycopy(strDihedPN[str], 0, newDihedPN, 0,
		    strDihedNumTerms[str]);
	    System.arraycopy(strDihedWeight[str], 0, newDihedWeight, 0,
		    strDihedNumTerms[str]);
	    System.arraycopy(strDihedLocalNum[str], 0, newDihedLocalNum, 0,
		    strDihedNumTerms[str]);
	    strDihedPN[str] = newDihedPN;
	    strDihedWeight[str] = newDihedWeight;
	    strDihedLocalNum[str] = newDihedLocalNum;
	...
	/*
	 * // LIGAND DIHEDRALS for(int i=0;i<numLigDihedrals;i++) { // Get
	 * center two atoms atom2 = ligDihedralAtNums[i][1]; atom3 =
	 * ligDihedralAtNums[i][2]; for(int q=0;q<m.connected[atom2][0];q++) {
	 * if (m.connected[atom2][q+1] != atom3) { for(int
	 * w=0;w<m.connected[atom3][0];w++) { if (m.connected[atom3][w+1] !=
	 * atom2) { // At this point 'q',atom2,atom3,'w' is a dihedral if
	 * (!(a96ff
	 * .getTorsionParameters(m.atom[m.connected[atom2][q+1]].type,m.atom
	 * [atom2].type,
	 * m.atom[atom3].type,m.atom[m.connected[atom3][w+1]].type,
	 * fConst,eqAngle,terms,multiplic))) {
	 * System.out.println("WARNING: Could not find torsion parameters for "
	 * + m.connected[atom2][q+1] + "," + atom2 + "," + atom3 +"," +
	 * m.connected[atom3][w+1]); return false; } for(int
	 * r=0;r<=multiplic[0];r++) { lDihedNumTerms++; if (lDihedNumTerms >
	 * tmpLSize) { // increase the size of the arrays newDihedPN = new
	 * int[tmpLSize + 500]; newDihedWeight = new double[tmpLSize + 500];
	 ...
	 * lDihedPN = newDihedPN; lDihedWeight = newDihedWeight; lDihedLocalNum
	 * = newDihedLocalNum; tmpLSize += 500; } lDihedPN[lDihedNumTerms] =
	 * terms[r]; lDihedWeight[lDihedNumTerms] = fConst[r];
	 * lDihedLocalNum[lDihedNumTerms] = i; } } } } } }
	 * 
	 * // Shrink the size of the array newDihedPN = new int[lDihedNumTerms];
	 * newDihedWeight = new double[lDihedNumTerms]; newDihedLocalNum = new
	 * int[lDihedNumTerms];
	...
	for (int str = 0; str < numberOfStrands; str++) {
		...
		dihedE += strDihedWeight[str][i]
			* (1 - Math
				.cos(strDihedPN[str][i]
					* strCumulativeDihedStep[str][strDihedLocalNum[str][i]]
					* d2r));
	    }
	}

	/*
	 * for(int i=0;i<sDihedNumTerms;i++){ dihedE += sDihedWeight[i] * (1 -
	 * Math.cos(sDihedPN[i]*sysCumulativeDihedStep[sDihedLocalNum[i]]*d2r));
	 * } // Ligand dihedral terms for(int i=0;i<lDihedNumTerms;i++){ dihedE
	 * += lDihedWeight[i] * (1 -
	 * Math.cos(lDihedPN[i]*ligCumulativeDihedStep[lDihedLocalNum[i]]*d2r));
	 * }
	 */
    ...
    // given dihedral; the computation is done in
    // computeOneDihedEnergyDiffHelper()
    public double computeOneDihedEnergyDiff(int strNumber, int dihedNumForCur,
	    double dihedChange) {
    ...
    /*
     * public double computeOneDihedEnergyDiff(boolean isLigDihed, int
     * dihedNumForCur, double dihedChange){
     * 
     * int sDNum, lDNum; double sDChange, lDChange; if (!isLigDihed) { //system
     * dihedral sDNum = dihedNumForCur; sDChange = dihedChange; lDNum = -1;
     * lDChange = 0.0; } else { //ligand dihedral sDNum = -1; sDChange = 0.0;
     * lDNum = dihedNumForCur; lDChange = dihedChange; }
     * 
     ...
     * 
     * double dihedE = 0.0; double d2r = 3.14159265 / 180.0;
     * 
     * // System dihedral terms first if (sDNum >= 0) { for(int
     * i=0;i<sDihedNumTerms;i++) { if (sDihedLocalNum[i] == sDNum) dihedE -=
     * sDihedWeight[i] * (1 - Math.cos(sDihedPN[i]*(sDChange)*d2r)); } } //
     * Ligand dihedral terms if (lDNum >= 0) { for(int i=0;i<lDihedNumTerms;i++)
     * { if (lDihedLocalNum[i] == lDNum) dihedE -= lDihedWeight[i] * (1 -
     * Math.cos(lDihedPN[i]*(lDChange)*d2r)); } }
     * 
     * return(dihedE); }
     */
	...
	result[1] = m.actualCoordinates[at3Base + 1]
		- m.actualCoordinates[at2Base + 1];
	result[2] = m.actualCoordinates[at3Base + 2]
		- m.actualCoordinates[at2Base + 2];
	...
	for (int i = 5; i < diArray[1] + 2; i++) {
	    ...
	    R[1] = m.actualCoordinates[atBase + 1]
		    - m.actualCoordinates[at3Base + 1];
	    R[2] = m.actualCoordinates[atBase + 2]
		    - m.actualCoordinates[at3Base + 2];
    ...
    /*
     * private void doLigTransRot(double ligTorque[], double ligTrans[], double
     * lligRotStep, double lligTransStep, double lligMaxTrans){
     * 
     * float bckpLigCoords[] = backupLigCoord(); //backup actual ligand
     * coordinates double initE[] =
     * a96ff.calculateTotalEnergy(m.actualCoordinates, -1); //compute energy
     * before translation/rotation
     * 
     * a96ff.calculateGradient(-1); //calculate the gradient (perhaps eventually
     * just calculate part of the gradient)
     * //a96ff.calculateEVGradientPartWithArrays(ligResNumPP);
     * computeLigTorqueTrans(ligTorque, ligTrans);
     * applyLigTorqueTrans(ligTorque, lligRotStep, ligTrans, lligTransStep,
     * lligMaxTrans);
     * 
     * double minE[] = a96ff.calculateTotalEnergy(m.actualCoordinates, -1);
     * //compute energy after translation/rotation if (initE[0]<minE[0]) {
     * //restore initial ligand actual coordinates if trans/rot increses energy
     * restoreLigCoord(bckpLigCoords); } }
     */

    private void doStrTransRot(int strNumber, double strTorque[],
	    double strTrans[], double RotStep, double TransStep, double MaxTrans) {

	float bckpStrCoords[] = backupStrCoord(strNumber); // backup actual
							   // ligand coordinates
	double initE[] = a96ff.calculateTotalEnergy(m.actualCoordinates, -1); // compute
									      // energy
									      // before
									      // translation/rotation

	a96ff.calculateGradient(-1); // calculate the gradient (perhaps
				     // eventually just calculate part of the
				     // gradient)
	// a96ff.calculateEVGradientPartWithArrays(ligResNumPP);
	computeStrTorqueTrans(strNumber, strTorque, strTrans);
	applyStrTorqueTrans(strNumber, strTorque, RotStep, strTrans, TransStep,
		MaxTrans);

	double minE[] = a96ff.calculateTotalEnergy(m.actualCoordinates, -1); // compute
									     // energy
									     // after
									     // translation/rotation
	// m.saveMolecule("minSingle.pdb", 0.0f);
	if (initE[0] <= minE[0]) { // restore initial ligand actual coordinates
				   // if trans/rot increses energy
	    restoreStrCoord(strNumber, bckpStrCoords);
	}
    }

    // Computes the torque around the ligand
    public void computeStrTorqueTrans(int strNumber, double strTorque[],
	    double strTrans[]) {
    ...
    /*
     * // Computes the torque around the ligand public void
     * computeLigTorqueTrans(double ligTorque[], double ligTrans[]){
     * 
     * Strand thisStrand = m.strand[ligStrNum]; double centOfMass[] =
     * m.getStrandCOM(ligStrNum); int atBase = -1; double R[] = new double[3];
     * double F[] = new double[3]; double temp[]; ligTorque[0] = 0.0;
     * ligTorque[1] = 0.0; ligTorque[2] = 0.0; ligTrans[0] = 0.0; ligTrans[1] =
     * 0.0; ligTrans[2] = 0.0;
     * 
     * // loop over all involved atoms for(int
     * i=0;i<thisStrand.numberOfResidues;i++){ for(int
     * j=0;j<thisStrand.residue[i].numberOfAtoms;j++){ atBase =
     * thisStrand.residue[i].atom[j].moleculeAtomNumber * 3; // R is the vector
     * from the COM to the current atom R[0] = m.actualCoordinates[atBase] -
     * centOfMass[0]; R[1] = m.actualCoordinates[atBase+1] - centOfMass[1]; R[2]
     * = m.actualCoordinates[atBase+2] - centOfMass[2]; // F is the current
     * gradient for the atom of interest F[0] = m.gradient[atBase]; F[1] =
     * m.gradient[atBase+1]; F[2] = m.gradient[atBase+2]; temp =
     * crossProduct(R,F); ligTorque[0] += temp[0]; ligTorque[1] += temp[1];
     * ligTorque[2] += temp[2]; ligTrans[0] += F[0]; ligTrans[1] += F[1];
     * ligTrans[2] += F[2]; } }
     * 
     * }
     */
    ...
    private void applyStrTorqueTrans(int strNumber, double strTorque[],
	    double RotStep, double strTrans[], double TransStep, double MaxTrans) {
	...
	theTranslation[0] = (float) (strStartCOM[strNumber][0]
		+ totalMovement[0] - strCurCOM[strNumber][0]);
	theTranslation[1] = (float) (strStartCOM[strNumber][1]
		+ totalMovement[1] - strCurCOM[strNumber][1]);
	theTranslation[2] = (float) (strStartCOM[strNumber][2]
		+ totalMovement[2] - strCurCOM[strNumber][2]);
	...
	if (mag != 0.0) {
		...
		m.rotateStrandAroundCOM(strNumber, (float) strTorque[0],
			(float) strTorque[1], (float) strTorque[2],
			(float) RotStep, false);
	}
	// Apply translation, don't update atom coordinates
	// The last parameter says not to update the atom coordintes,
	// only update the actualCoords
	m.translateStrand(strNumber, theTranslation[0], theTranslation[1],
     ...
     * 
     * // normalize the ligand translation // ligTrans will now have magnitude
     * lligTransStep // we change sign because the gradient points uphill double
     * scale = getMagnitude(ligTrans)/lligTransStep; if (scale != 0.0) {
     * ligTrans[0] /= -scale; ligTrans[1] /= -scale; ligTrans[2] /= -scale; }
     * 
     * // determine the new COM if we took this step double tempCOM[] = new
     * double[3]; tempCOM[0] = ligCurCOM[0] + ligTrans[0]; tempCOM[1] =
     * ligCurCOM[1] + ligTrans[1]; tempCOM[2] = ligCurCOM[2] + ligTrans[2];
     * 
     * // compute how large of a step this would be from the // start point
     * double totalMovement [] = new double[3]; totalMovement[0] = tempCOM[0] -
     * ligStartCOM[0]; totalMovement[1] = tempCOM[1] - ligStartCOM[1];
     * totalMovement[2] = tempCOM[2] - ligStartCOM[2]; scale =
     * getMagnitude(totalMovement);
     * 
     * // if the step would take us too far away then // scale it back if (scale
     * > lligMaxTrans){ scale = scale / lligMaxTrans; totalMovement[0] /= scale;
     * totalMovement[1] /= scale; totalMovement[2] /= scale; }
     * 
     * // compute the translation to get us to the new COM float
     * theTranslation[] = new float[3]; theTranslation[0] =
     * (float)(ligStartCOM[0] + totalMovement[0] - ligCurCOM[0]);
     * theTranslation[1] = (float)(ligStartCOM[1] + totalMovement[1] -
     * ligCurCOM[1]); theTranslation[2] = (float)(ligStartCOM[2] +
     * totalMovement[2] - ligCurCOM[2]);
     * 
     * // update the current COM ligCurCOM[0] = ligStartCOM[0]+totalMovement[0];
     * ligCurCOM[1] = ligStartCOM[1]+totalMovement[1]; ligCurCOM[2] =
     * ligStartCOM[2]+totalMovement[2];
     * 
     * // Apply torque // Since our step size is fixed we just need to know the
     * // vector to rotate around // we change sign because the gradient points
     * uphill double mag = getMagnitude(ligTorque); if (mag != 0.0) {
     * ligTorque[0] /= -mag; ligTorque[1] /= -mag; ligTorque[2] /= -mag; // The
     * last parameter says not to update the atom coordintes, // only update the
     * actualCoords if (Math.abs(lligRotStep)>0.0001) // this should be in
     * m.rotateStrandAroundCOM
     * (ligStrNum,(float)ligTorque[0],(float)ligTorque[1],
     * (float)ligTorque[2],(float)lligRotStep,false); } // Apply translation,
     * don't update atom coordinates // The last parameter says not to update
     * the atom coordintes, // only update the actualCoords
     * m.translateStrand(ligStrNum
     * ,theTranslation[0],theTranslation[1],theTranslation[2],false);
     * 
     * }
     */
    ...
    private float computeDihedDiff(int diAtArray[], int atomList[], int alSize,
	    int AANum, float stepSize, int strNumber, int dihedNumForCur) {
	...
	if (doDihedEnergy)
	    initialEnergy[0] += computeOneDihedEnergyDiff(strNumber,
		    dihedNumForCur, 0.0f);

	// Apply a rotation
	// at1, at2, and at3 don't move, at4 and the atoms
	// in the atomList rotate
	m.changeTorsion(diAtArray[0], diAtArray[1], diAtArray[2], diAtArray[3],
		stepSize, atomList, alSize, false);

	// Compute second energy
	secondEnergy = a96ff.calculateTotalEnergy(m.actualCoordinates, AANum);
	if (doDihedEnergy) {
	    secondEnergy[0] += computeOneDihedEnergyDiff(strNumber,
		    dihedNumForCur, cumulStep + stepSize);
	}
	...
	if (doDihedEnergy)
	    thirdEnergy[0] += computeOneDihedEnergyDiff(strNumber,
		    dihedNumForCur, cumulStep - stepSize);
	...
	if ((initialEnergy[0] > secondEnergy[0])
		&& (initialEnergy[0] > thirdEnergy[0])) {
    ...
    // This function updates the total amount (degrees) in which the given
    // dihedral
    // has moved. It limits this movement to be within +/- lmaxMovement
    private void updateCumulative(double cumulativeDihedStep[],
	    double newDihedDiff[], int index, double lmaxMovement) {
    ...
    // Uses specific precomputed nonbonded arrays for each residue (this makes
    // things
    // run faster)
	...
	/*
	 * if ((ligandOnly)&&(ligStrNum == -1)){ return; }
	 */
	...
	/*
	 * if(ligStrNum != -1){ // get the staring COM ligStartCOM =
	 * m.getStrandCOM(ligStrNum); ligCurCOM[0] = ligStartCOM[0];
	 * ligCurCOM[1] = ligStartCOM[1]; ligCurCOM[2] = ligStartCOM[2]; }
	 */
	...
	/*
	 * for(int j=0;j<numSysDihedrals;j++){ sysDihedDiff[j] = 0.0;
	 * sysCumulativeDihedStep[j] = 0.0; } for(int
	 * j=0;j<numLigDihedrals;j++){ ligDihedDiff[j] = 0.0;
	...
	/*
	 * if(ligStrNum != -1)
	 * a96ff.setupPartialArrays(numFlexRes+2,MAX_NUM_ATOMS_DISTAL
	 * ,flexResAtomList, flexResListSize); else
	 */
	a96ff.setupPartialArrays(totalFlexRes + totalTransRotStrands,
		MAX_NUM_ATOMS_DISTAL, flexResAtomList, flexResListSize);

	while (!done) {

	    for (int str = 0; str < numberOfStrands; str++) {
		for (int j = 0; j < numStrDihedrals[str]; j++) {
		    strDihedDiff[str][j] = computeDihedDiff(
			    strDihedralAtNums[str][j],
			    strDihedralDistal[str][j],
			    strNumAtomsDistal[str][j],
			    strDihedToResNum[str][j], step, str, j);
		    updateCumulative(strCumulativeDihedStep[str],
			    strDihedDiff[str], j, lmaxMovement);
		    applyDihedStep(strDihedralAtNums[str][j],
			    strDihedralDistal[str][j],
			    strNumAtomsDistal[str][j], strDihedDiff[str][j]);
		}
	    }

	    /*
	     * for(int j=0;j<numLigDihedrals;j++) { ligDihedDiff[j] =
	     * computeDihedDiff(ligDihedralAtNums[j],ligDihedralDistal[j],
	     * ligNumAtomsDistal[j],ligResNumber,step,true,j);
	     * updateCumulative(ligCumulativeDihedStep
	     * ,ligDihedDiff,j,lmaxMovement);
	     * applyDihedStep(ligDihedralAtNums[j]
	     * ,ligDihedralDistal[j],ligNumAtomsDistal[j],ligDihedDiff[j]); }
	     */

	    // Translate and rotate the ligand
	    /*
	     * if(ligStrNum != -1) doLigTransRot(ligTorque, ligTrans,
	     * lligRotStep, lligTransStep, lligMaxTrans);
	     */

	    for (int str = 0; str < numberOfStrands; str++) {
		if (m.strand[str].rotTrans) {
		    doStrTransRot(str, strTorque, strTrans, strRotStep,
			    strTransStep, strMaxTrans);
		}
	    }

	    /*
	     * if(debug){ String filename =
	     * String.format("badVdw/run%1$d/structure%2$02d.pdb",GLOBALNUM,i);
	...
	clearMolGradient(); // after minimization is done, clear the molecule
			    // gradient
    ...
    // Backup the actual ligand coordinates (assumes there is only 1 residue in
    // the ligand strand)
    ...
    // Restore the backup actual ligand coordinates (assumes there is only 1
    // residue in the ligand strand)
    ...
    /*
     * private float [] backupLigCoord(){ float bckpLigCoords[] = new
     * float[m.strand[ligStrNum].numberOfAtoms*3]; for (int i=0;
     * i<m.strand[ligStrNum].numberOfAtoms; i++){ int curLigAtom =
     * m.strand[ligStrNum].residue[0].atom[i].moleculeAtomNumber;
     * bckpLigCoords[i*3] = m.actualCoordinates[curLigAtom*3];
     * bckpLigCoords[i*3+1] = m.actualCoordinates[curLigAtom*3+1];
     * bckpLigCoords[i*3+2] = m.actualCoordinates[curLigAtom*3+2]; } return
     * bckpLigCoords; }
     * 
     * //Restore the backup actual ligand coordinates (assumes there is only 1
     * residue in the ligand strand) private void restoreLigCoord(float
     * bckpLigCoords[]){ for (int i=0; i<m.strand[ligStrNum].numberOfAtoms;
     * i++){ int curLigAtom =
     * m.strand[ligStrNum].residue[0].atom[i].moleculeAtomNumber;
