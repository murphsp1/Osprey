 ...
 * force field parameters are read in and saved upon initialization. The EEF1
 * solvation parameters are also read in using the EEF1 class functions. The
 * energy (including electrostatic, vdW, and solvation) and gradient computation
 * for the full molecule or a selected subset of the molecule atoms is performed
 * by this class.
 */
public class Amber96ext implements ForceField, Serializable {
	...
	final int atomTypeX = -2; // the atom type number for the X wildcard atom
	// type
	...
	int numGeneralDihedParams = 0; // the number of generic dihedral parameters
	// (that have wildcard X atoms)
	...
	Amber96ext(Molecule m, boolean ddDielect, double dielectConst,
		...
		try {
			...
			System.out
			.println("ERROR: An error occurred while reading a template file: "
					+ e);
			System.exit(0);
		}
	...
	//********
	//this function is a complete hack, a fast inverse square root computation
	...
	private void readParm96() throws Exception {
		...
		FileInputStream is = new FileInputStream(EnvironmentVars.getDataDir()
				.concat(amberDatInFile));
		BufferedReader bufread = new BufferedReader(new InputStreamReader(is));
		String curLine = null, tmpStr = null;
		int tmpInt = 0;

		final int initSize = 10; // the initial size of the arrays to store the
		// data that is read
		...
		while (!(getToken(curLine, 1).equals(""))) {
			...
			atomAtomicMasses[tmpInt] = (new Float(getToken(curLine, 2)))
					.floatValue();
			tmpInt++;
			curLine = bufread.readLine();
		}
		...
		while (!(getToken(curLine, 1).equals(""))) {
			...
			bondHFC[tmpInt] = (new Float(getDashedToken(curLine, 3)))
					.floatValue();
			bondEBL[tmpInt] = (new Float(getDashedToken(curLine, 4)))
					.floatValue();
			tmpInt++;
			curLine = bufread.readLine();
		}
		...
		while (!(getToken(curLine, 1).equals(""))) {
			...
			angleHFC[tmpInt] = (new Float(getDashedToken(curLine, 4)))
					.floatValue();
			angleEBA[tmpInt] = (new Float(getDashedToken(curLine, 5)))
					.floatValue();
			tmpInt++;
			curLine = bufread.readLine();
		}
		...
		while (!(getToken(curLine, 1).equals(""))) {
			...
			if (dihedAtomType1[tmpInt] == atomTypeX
					|| dihedAtomType2[tmpInt] == atomTypeX
					|| dihedAtomType3[tmpInt] == atomTypeX
					|| dihedAtomType4[tmpInt] == atomTypeX) // at least one of
				// the atoms is a
				// wildcard
				numGeneralDihedParams++;

			tmpFlt = (new Float(getDashedToken(curLine, 5))).floatValue();
			dihedTerm1[tmpInt] = (new Float(getDashedToken(curLine, 6)))
					.floatValue() / tmpFlt;
			dihedPhase[tmpInt] = (new Float(getDashedToken(curLine, 7)))
					.floatValue();
			dihedPN[tmpInt] = (new Float(getDashedToken(curLine, 8)))
					.intValue();
			// If dihedPN is negative then there are one or more additional
			// terms
			// nothing fancy needs to be done because they will all be read in
			// anyway
		...
		while (!(getToken(curLine, 1).equals(""))) {
			...
			impDihedTerm1[tmpInt] = (new Float(getDashedToken(curLine, 5)))
					.floatValue();
			impDihedPhase[tmpInt] = (new Float(getDashedToken(curLine, 6)))
					.floatValue();
			impDihedPN[tmpInt] = (new Float(getDashedToken(curLine, 7)))
					.intValue();
			tmpInt++;
			curLine = bufread.readLine();
		}
		...
		// The first atomnum in equivAtoms is the main atom and numbers with
		// index 1..n are
		...
		while (!(getToken(curLine, 1).equals(""))) {
			...
			while (!getToken(curLine, tmpInt2).equalsIgnoreCase("")) {
				equivAtoms[tmpInt][tmpInt2 - 1] = atomTypeToInt(getToken(
						curLine, tmpInt2));
				tmpInt2++;
			}
			tmpInt++;
		...
		/*
		 * System.out.println("ATOM TYPES"); for(int q=0;q<5;q++) {
		 * System.out.println(q + " " + atomTypeNames[q] + " mass:" +
		 * atomAtomicMasses[q]); } System.out.println("BONDS"); for(int
		 * q=0;q<5;q++) { System.out.println(q + " " + bondAtomType1[q] + " " +
		 * bondAtomType2[q] + " HFC:" + bondHFC[q] + " EBL:" + bondEBL[q]); }
		 * System.out.println("ANGLES"); for(int q=0;q<5;q++) {
		 * System.out.println(q + " " + angleAtomType1[q] + " " +
		 * angleAtomType2[q] + " " + angleAtomType3[q] + " HFC:" + angleHFC[q] +
		 * " EBA:" + angleEBA[q]); } System.out.println("DIHED"); for(int
		 * q=0;q<5;q++) { System.out.println(q + " " + dihedAtomType1[q] + " " +
		 * dihedAtomType2[q] + " " + dihedAtomType3[q] + " " + dihedAtomType4[q]
		 * + " term1:" + dihedTerm1[q] + " PN:" + dihedPN[q] + " Phase:" +
		 * dihedPhase[q]); } System.out.println("IMPROP DIHED"); for(int
		 * q=0;q<5;q++) { System.out.println(q + " " + impDihedAtomType1[q] +
		 * " " + impDihedAtomType2[q] + " " + impDihedAtomType3[q] + " " +
		 * impDihedAtomType4[q] + " term1:" + impDihedTerm1[q] + " PN:" +
		 * impDihedPN[q] + " Phase:" + impDihedPhase[q]); }
		 * System.out.println("VDW"); for(int q=0;q<5;q++) {
		 * System.out.println(q + " " + vdwAtomType1[q] + " " + " R:" + vdwR[q]
		 * + " E:" + vdwE[q]); } // DEBUG END
		 */
	...
	public boolean getStretchParameters(int atomType1, int atomType2,
		...
		for (int q = 0; q < bondAtomType1.length; q++) {
			if (match2Atoms(atomType1, atomType2, bondAtomType1[q],
					bondAtomType2[q])) {
				tmpInt = 0;
				matched = true;
	...
	// and returns the approprate constants
	public boolean getBendParameters(int atomType1, int atomType2,
			int atomType3, double forceConstant[], double equilibriumAngle[]) {
		...
		if (matched) {
			...
			System.out
			.println("AmberBend: Could not find correct angle, defaulting to CC-CT-CT");
			return (false);
		}
	}
	...
	// and returns the approprate constants
	public boolean getTorsionParameters(int atomType1, int atomType2,
			int atomType3, int atomType4, double forceConstant[],
			double equilibriumAngle[], int terms[], int multiplicity[]) {
		...
		// First search through generic torsions (those terms containing
		// wildcards)
		boolean matched = false;
		for (int q = 0; q < numGeneralDihedParams; q++) {
			if (match2Atoms(atomType2, atomType3, dihedAtomType2[q],
					dihedAtomType3[q])) {
				matched = true;
		...
		// According to the original paper "any specific parameter, such as
		// OS-CH-CH-OS,
		...
		for (int q = numGeneralDihedParams; q < dihedAtomType1.length; q++) {
			if (match4Atoms(atomType1, atomType2, atomType3, atomType4,
					dihedAtomType1[q], dihedAtomType2[q], dihedAtomType3[q],
					dihedAtomType4[q])) {
				matched = true;
	...
	// This function attempts to match four atoms for a dihedral (no generic
	// atoms)
	private boolean match4Atoms(int atType1, int atType2, int atType3,
			int atType4, int known1, int known2, int known3, int known4) {
	...
	public boolean getNonBondedParameters(int atomType, double r[],
			double epsilon[]) {
	...
	// If residues do match, the difference between the number of atoms in the
	// template
	// and res is returned
	private int checkAAType(Molecule m, Residue templateRes, Residue res,
			int[] atArray) {
		...
		// Require all hydrogens to match (last parameter of recurseAA() is
		// 'true')
	...
	public boolean assignAA(Molecule m, int nr) {
		...
		for (int i = 0; i < aat.numAAs; i++) {
			if (aat.aaResidues[i].name.equalsIgnoreCase(res.name)) {
				if (d != noMatchInt)
					System.out
					.println("** Residue already matched to nonterm template, this should not happen **");
		...
		for (int i = 0; i < aat.numAANTs; i++) {
			if (aat.aaNTResidues[i].name.equalsIgnoreCase(res.name)) {
				if (dNT != noMatchInt)
					System.out
					.println("** Residue already matched to NT template, this should not happen **");
		...
		for (int i = 0; i < aat.numAACTs; i++) {
			if (aat.aaCTResidues[i].name.equalsIgnoreCase(res.name)) {
				if (dCT != noMatchInt)
					System.out
					.println("** Residue already matched to CT template, this should not happen **");
		...
		if ((d < dNT) && (d < dCT)) {
			...
			atA = atArray;
			if (debug)
				System.out.println("Residue " + nr
						+ " matched to non-terminal: " + res.name);
		} else if (dNT < dCT) {
			tR = templateResNT;
			atA = atArrayNT;
			tR.cterm = false;
			tR.nterm = true;
			if (debug)
				System.out.println("Residue " + nr + " matched to N-terminal: "
						+ res.name);
		} else {
			tR = templateResCT;
			tR.cterm = true;
			...
			atA = atArrayCT;
			if (debug)
				System.out.println("Residue " + nr + " matched to C-terminal: "
						+ res.name);
		}
		for (int q = 0; q < tR.numberOfAtoms; q++) {
			if (atA[q] != -1) {
				res.atom[atA[q]].forceFieldType = tR.atom[q].forceFieldType;
				// System.out.println(" FFT: " + atA[q] + " " +
				// tR.atom[q].forceFieldType + " charge: " + tR.atom[q].charge);
				res.atom[atA[q]].charge = tR.atom[q].charge;
				assignNumericalType(res.atom[atA[q]],
						res.atom[atA[q]].forceFieldType);
			}
		}
		res.cterm = tR.cterm;
		res.nterm = tR.nterm;
	...
	private boolean recurseAA(Molecule m, Residue templateRes, Residue res,
			int[] atArray, boolean[] atUsedArray, int curAtResNum,
			boolean requireHydrogens) {
				...
				if (res.atom[w].elementType.equalsIgnoreCase(templEleType)) {
					...
					for (int k = 0; k < m.connected[curAtMolNum][0]; k++) {
						...
						if (tmpAt.residueAtomNumber == atArray[rootNum]) {
							...
							if (recurseAA(m, templateRes, res, atArray,
									atUsedArray, curAtResNum + 1,
									requireHydrogens)) {
								if (debug)
		...
		// if we got here then no assignment worked, but we allow hydrogens to
		// slip
		// so if this is a hydrogen atom then we let it slide
		if (templEleType.equalsIgnoreCase("H") && !requireHydrogens)
			if (recurseAA(m, templateRes, res, atArray, atUsedArray,
					curAtResNum + 1, requireHydrogens)) {
				return true;
			}
	...
	// template
	// and res is returned
	private int checkGRType(Molecule m, Residue templateRes, Residue res,
			int[] atArray) {
		...
		// Find the anchor atom for each generic residue in the input PDB file:
		// test all atoms;
		...
		for (int q = 0; q < res.numberOfAtoms; q++) {
			...
			// Note that we can use recurseAA because that function is NOT AA
			// specific
			if (recurseAA(m, templateRes, res, atArray, atUsedArray, 1, true)) {
				int d = (templateRes.numberOfAtoms - res.numberOfAtoms);
				if (d < 0)
	...
	public boolean assignGR(Molecule m, int nr) {
		...
		for (int i = 0; i < grt.numGRs; i++) {
			if (grt.grResidues[i].name.equalsIgnoreCase(res.name)) {
				if (d != noMatchInt)
					System.out
					.println("** Residue already matched to template, this should not happen **");
		...
		for (int q = 0; q < templateRes.numberOfAtoms; q++) {
			if (atArray[q] != -1) {
				...
				// System.out.println(" FFT: " + atArray[q] + " " +
				// templateRes.atom[q].forceFieldType + " charge: " +
				// templateRes.atom[q].charge);
				res.atom[atArray[q]].charge = templateRes.atom[q].charge;
				assignNumericalType(res.atom[atArray[q]],
						res.atom[atArray[q]].forceFieldType);
			}
		}
		res.ffAssigned = true;
		return true;
		...
		for (int q = 0; q < m.numberOfStrands; q++) {
			if (m.strand[q].isProtein) {
				for (int w = 0; w < m.strand[q].numberOfResidues; w++) {
					// if ( m.strand[q].residue[w].getEnergyEvalSC() ||
					// m.strand[q].residue[w].getEnergyEvalBB() ){
					Residue res = m.strand[q].residue[w];
					if (res.cofactor) { // KER: I allow cofactor residues in a
						// protein strand cause they will never
						// move/rotate
						if (!calculateOneGRWithTemplates(res)) {
							System.out
							.println("WARNING: UNABLE TO FIND GENERIC TEMPLATE FOR: "
									+ m.strand[q].residue[w].fullName);
						} else { // we were able to assign types with template
							// System.out.println("Assigned strand: " + q +
							// " res: " + w + " to an AAT");
							// Make sure all atoms were assigned, if not
							// assign them by standard methods
							for (int z = 0; z < res.numberOfAtoms; z++) {
								if (res.atom[z].forceFieldType.equals("")) {
									System.out.println(" patching res: " + w
											+ " atom: " + z + " "
											+ res.atom[z].elementType + " "
											+ res.atom[z].name);
									System.out
									.println("WARNING: Unable to patch residue\n");
								}
							}
						}
					} else {
						if (!calculateOneAAWithTemplates(res)) {
							System.out
							.println("WARNING: UNABLE TO FIND AA TEMPLATE FOR: "
									+ m.strand[q].residue[w].fullName);
						} else { // we were able to assign types with template
							// System.out.println("Assigned strand: " + q +
							// " res: " + w + " to an AAT");
				...
				for (int w = 0; w < m.strand[q].numberOfResidues; w++) {
					...
					if (!calculateOneGRWithTemplates(res)) {
						...
						// System.out.println("Assigned strand: " + q +
						// " res: " + w + " to an AAT");
						// Make sure all atoms were assigned, if not assign
						// them by standard methods
						for (int z = 0; z < res.numberOfAtoms; z++) {
							if (res.atom[z].forceFieldType.equals("")) {
								System.out.println(" patching res: " + w
										+ " atom: " + z + " "
	...
	public void setNBEval(boolean electEval, boolean vdwEval) {
		...
		for (int i = 0; i < numHalfNonBondedTerms; i++) {
			if (m.atom[(int) halfNonBondedTerms[i * 4]].elementType
					.equalsIgnoreCase("H"))
				halfNBeval[i] = evalNum;
			else {
				if (m.atom[(int) halfNonBondedTerms[i * 4 + 1]].elementType
						.equalsIgnoreCase("H"))
					halfNBeval[i] = evalNum;
				else
					halfNBeval[i] = 1;
		...
		for (int i = 0; i < numberNonBonded; i++) {
			if (m.atom[(int) nonBondedTerms[i * 4]].elementType
					.equalsIgnoreCase("H"))
				NBeval[i] = evalNum;
			else {
				if (m.atom[(int) nonBondedTerms[i * 4 + 1]].elementType
						.equalsIgnoreCase("H"))
					NBeval[i] = evalNum;
				else
					NBeval[i] = 1;
	...
	public void initializeCalculation() {
		...
		initializeEVCalculation(); // initialize the calculation of the
	...
	private void initializeEVCalculation() {
			...
			if (evalAtom[atom1] && evalAtom[atom4]) {
				...
				if (!(getNonBondedParameters(atomType1, equilibriumDistance, epsilon)))
					...
					.println("WARNING: Could not find nb parameters for "
							+ atom1
							+ " type: "
							+ m.atom[atom1].forceFieldType);
				else {
					if ((EnvironmentVars.forcefld == EnvironmentVars.FORCEFIELD.CHARMM19 || EnvironmentVars.forcefld == EnvironmentVars.FORCEFIELD.CHARMM19NEUTRAL)
							&& m.atom[m.connected14[ix4]].elementType
							.equalsIgnoreCase("C")) {
						// KER: if charmm19 then reduce C radii for 1-4
						// interactions
						epsilonProduct = 0.1;
						ri = 1.9;
					} else {
					...
					if (!(getNonBondedParameters(atomType4,
							equilibriumDistance, epsilon)))
						System.out
						.println("WARNING: Could not find nb parameters for "
								+ atom4
								+ " type: "
								+ m.atom[atom4].forceFieldType);
					else {
						if ((EnvironmentVars.forcefld == EnvironmentVars.FORCEFIELD.CHARMM19 || EnvironmentVars.forcefld == EnvironmentVars.FORCEFIELD.CHARMM19NEUTRAL)
								&& m.atom[m.connected14[ix4]].elementType
								.equalsIgnoreCase("C")) {
		...
		if (debug)
			System.out.println("Final number of halfNonBondedTerms: "
					+ numHalfNonBondedTerms);
		...
		for (int i = 0; i < m.numberNonBonded; i++) {
			...
			if (evalAtom[atom1] && evalAtom[atom2]) {
				...
				if (!(getNonBondedParameters(atomType1, equilibriumDistance,
						epsilon)))
					System.out
					.println("WARNING: Could not find nb parameters for (at1) "
							+ atom1
							+ " type: "
							+ m.atom[atom1].forceFieldType);
				else {
					...
					if (!(getNonBondedParameters(atomType2,
							equilibriumDistance, epsilon)))
						System.out
						.println("WARNING: Could not find nb parameters for (at2) "
								+ atom2
								+ " type: "
								+ m.atom[atom2].forceFieldType);
					else {
						epsilonProduct *= epsilon[0];
						float rj = (float) equilibriumDistance[0];
						epsilonProduct = Math.sqrt(epsilonProduct);
						float Bij = (float)((ri + rj) * (ri + rj));
						Bij = Bij * Bij * Bij;
		...
		if (debug)
			System.out.println("Final number of full nonbonded pairs: "
					+ numberNonBonded);
	}
	...
	// not for cofactors. To determine if an atom belongs to a protein or a
	// ligand,
	...
	private void initializeSolvationCalculation() {
		...
		for (int i = 0; i < m.numberOfResidues; i++) {
			if (m.strand[m.residue[i].strandNumber].isProtein
					&& !m.residue[i].cofactor) // only compute solvation
				// energies for the protein and
				// an AA ligand
				evalAtom = getEvalForRes(m.residue[i], evalAtom);
		}
		...
		for (int i = 0; i < m.numberOfAtoms; i++) {
			...
			if (evalAtom[atom1]) {
				...
				if (!m.atom[atom1].elementType.equalsIgnoreCase("H")) {// solvation
					// terms
					// do not
					// include
					// H
					...
					double vdWradiusExt[] = new double[1]; // extended vdWradius
					// (uses the EEF1
					// parameters)

					if (!(eef1parms.getSolvationParameters(atom1, dGref,
							dGfree, atVolume, lambda, vdWradiusExt))) {
						System.out
						.println("WARNING: Could not find solvation parameters for atom: "
								+ atom1
								+ " ("
								+ m.atom[atom1].name
								+ ") res: "
								+ m.atom[atom1].moleculeResidueNumber
								+ " ("
								+ m.residue[m.atom[atom1].moleculeResidueNumber].name
								+ ")");
						System.exit(1);
					} else {
	...
	// Determines which atoms of residue res (molecule-relative numbering)
	// should be included in the energy computation
	private boolean[] getEvalForRes(Residue res, boolean evalAtom[]) {
		...
		if (res.getEnergyEvalSC()) {
			if (res.ffAssigned) {
				...
				System.out.println("ERROR: the force field for residue "
						+ res.fullName + " is not assigned");
				System.exit(1);
			}
		}
	...
	// ////////////////////////////////////////////////////////////////////////////////////////////////
	// Sets up partial atom arrays: used by SimpleMaximizer to quickly compute
	// energies on given subsets of
	// the full atoms arrays (nonBonded, halfNonBonded, dihedralAngleTerms,
	// etc.)
	// ////////////////////////////////////////////////////////////////////////////////////////////////
	// Sets up the partial arrays
	public void setupPartialArrays(int numRows, int maxNumColumns,
			int atomList[][], int numColumns[]) {

		setupPartialNonBondedArrays(numRows, maxNumColumns, atomList,
				numColumns); // setup nonbonded arrays

		if (doSolvationE) // setup dihedral arrays
			setupPartialSolvationArrays(numRows, maxNumColumns, atomList,
					numColumns);
	}

	// Sets up local datastructures to hold lists of the halfNonBonded and
	// nonBonded
	...
	private void setupPartialNonBondedArrays(int numRows, int maxNumColumns,
			int atomList[][], int numColumns[]) {
		...
		for (int i = 0; i < numRows; i++) {
			partHalfNonBonded[i] = new float[numColumns[i] * m.numberOfAtoms * 4];
	...
	private void setupPartialSolvationArrays(int numRows, int maxNumColumns,
			int atomList[][], int numColumns[]) {
	...
	// If (curIndex!=-1), then precomputed partial arrays are used to quickly
	// compute the specified energy terms
	public double[] calculateTotalEnergy(float coordinates[], int curIndex) {

		double energyTerms[] = new double[4]; // total, electrostatics, vdW, and
		// solvation
		for (int i = 0; i < energyTerms.length; i++)
			energyTerms[i] = 0.0;

		calculateEVEnergy(coordinates, curIndex, energyTerms); // compute
		// electrostatic
		// and vdW
		// energies
	...
	// This function calculates the electrostatic and vdw (EV) energy of a
	// system
	// Energy values are 'R'eturned in EenergyR and VenergyR
	// vdwMultiplier is the soft-potential multiplier, if equal to 1.0 it has no
	// effect,
	...
	private void calculateSolvationEnergy(float coordinates[], int curIndex,
		...
		if (curIndex == -1) // all residues included
			...
			// only residue curIndex included (partial matrices used)
			...
			for (int j = i + 1; j < numSolvationTerms; j++) { // the pairwise
				...
				if (!localSolvExcludePairs[j]) {
					...
					if (rij2 < solvCutoffSquared) {
						...
						//Sean added
						//double coeff = INV_OF_2_PI_SQRTPI;
	...
	public void calculateGradient() {
		System.out
		.println("ERROR: the function calculateGradient(int index) must be used.");
		System.exit(1);
	}
	...
	private void calculateEVGradient(int curIndex) {
		...
		if (curIndex == -1) { // full gradient is computed
			...
			numNBterms = numberNonBonded;
			nbTerms = nonBondedTerms;
		} else { // partial gradient is computed, based on flexible residue
			// curIndex
			numHalfNBterms = numPartHalfNonBonded[curIndex];
			halfNBterms = partHalfNonBonded[curIndex];
			numNBterms = numPartNonBonded[curIndex];
	...
	private void setForcefieldInputs() {
		...
		case CHARMM19NEUTRAL:
			...
			break;
		default:
			System.out.println("DON'T RECOGNIZE FORCEFIELD: "
					+ EnvironmentVars.forcefld.name());
			System.exit(0);
			break;
		}
