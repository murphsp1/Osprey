...
public class DEEGoldstein {
	...
	//constructor
	DEEGoldstein(float arpMatrix[][][][][][], float arpMatrixMax[][][][][][], int numResMutable, 
			...
			for (int AA=0; AA<numAAtypes[posNum]; AA++){
				...
				for (int altRot=0; altRot<numRotForAAatPos; altRot++){
					...
					if (!((altAA==AANumAtPos)&&(altRot==rotNumAtPos))){
						...
						//index_t =  strandRot[str].getIndex(strPos, altAA, altRot, strandMut);    //posNum*numTotalRot + rotIndOffset[altAA] + altRot;
	...
	private double IndMinDiffPVE (int firstPos, int firstAA1, int firstRot1, int firstAA2, int firstRot2, int secondPos){
		...
		//r at i
		//index1 = strandRot[str1].getIndex(strPos, firstAA1, firstRot1, strandMut);//firstPos*numTotalRot + rotIndOffset[firstAA1] + firstRot1;
		
		//t at i
		//index3 = strandRot[str1].getIndex(strPos, firstAA2, firstRot2, strandMut);//firstPos*numTotalRot + rotIndOffset[firstAA2] + firstRot2;
