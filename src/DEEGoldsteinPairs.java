...
public class DEEGoldsteinPairs {
	...
	//constructor
	DEEGoldsteinPairs(float arpMatrix[][][][][][], float arpMatrixMax[][][][][][], int numResMutable, 
			...
			int mutRes2StrandP[], int mutRes2MutIndexP[], boolean typeDep, boolean aIMinDEE, float aIval) {
		...
		if (minimizeBB) //backbone minimization, so we need the template interval energy (otherwise, templateInt will be 0.0)			
			templateInt = RotamerSearch.getShellShellE(pairwiseMaxEnergyMatrix)-RotamerSearch.getShellShellE(pairwiseMinEnergyMatrix);//pairwiseMaxEnergyMatrix[pairwiseMaxEnergyMatrix.length-1][0][0][0][0][0] - pairwiseMinEnergyMatrix[pairwiseMinEnergyMatrix.length-1][0][0][0][0][0];
					...
					for (int curPos2=curPos1+1; curPos2<numMutable; curPos2++){
						...
						if ((magicBullet)||(!distrDEE)||(resInPair[curPos2])){ //mb-pairs or not distrDEE or cur res is in distr pair
							...
							for (int AA1=0; AA1<numAAtypes[curPos1]; AA1++){
								...
								numRotForCurAAatPos1 = strandRot[str1].rl.getNumRotForAAtype(curAA1);
								...
								for(int curRot1=0; curRot1<numRotForCurAAatPos1; curRot1++){
									...
									if (!eliminatedRotAtPos.get(curPos1,curAA1,curRot1)){//not already pruned
										...
										for (int AA2=0; AA2<numAAtypes[curPos2]; AA2++){
											...
											int numRotForCurAAatPos2 = strandRot[str2].rl.getNumRotForAAtype(curAA2);
	...
	private boolean CanEliminate (int posNum1, int AANumAtPos1, int rotNumAtPos1, int posNum2, 
		...
		minShellResE = RotamerSearch.getShellRotE(pairwiseMinEnergyMatrix, posNum1, AANumAtPos1, rotNumAtPos1)+RotamerSearch.getShellRotE(pairwiseMinEnergyMatrix, posNum2, AANumAtPos2, rotNumAtPos2);//pairwiseMinEnergyMatrix[posNum1][AANumAtPos1][rotNumAtPos1][posNum1][0][1] + pairwiseMinEnergyMatrix[posNum2][AANumAtPos2][rotNumAtPos2][posNum2][0][1];
		...
		if ((!eliminatedRotAtPos.get(posNum1,AANumAtPos1,rotNumAtPos1))&&(!eliminatedRotAtPos.get(posNum2,AANumAtPos2,rotNumAtPos2))
			...
			for (int AA1=0; AA1<numAAtypes[posNum1]; AA1++){
				...
				numRotForAAatPos1 = strandRot[str1].rl.getNumRotForAAtype(altAA1);
						...
						if ((!eliminatedRotAtPos.get(posNum1,altAA1,altRot1))){ //not pruned
							...
							for (int AA2=0; AA2<numAAtypes[posNum2]; AA2++){
								...
								if(!typeDependent || (altAA1 == AANumAtPos1 && altAA2 == AANumAtPos2)){
									numRotForAAatPos2 = strandRot[str2].rl.getNumRotForAAtype(altAA2);
			...
			for (int AA=0; AA<numAAtypes[thirdPos]; AA++){
				...
				for (int curRot=0; curRot<numRotForAAatPos; curRot++){
					...
					if ((!eliminatedRotAtPos.get(index2))){ //not pruned 
						...
						if ((!useFlags)||((!splitFlags[firstPos][firstAA1][firstRot1][thirdPos][curAA][curRot])&&(!splitFlags[secondPos][secondAA1][secondRot1][thirdPos][curAA][curRot]))){ //not using split flags or not flagged
		...
		for (int AA1=0; AA1<numAAtypes[atPos1]; AA1++){
			...
			for (int curRot1=0; curRot1<numRotForAAatPos1; curRot1++){
				...
				for (int AA2=0; AA2<numAAtypes[atPos2]; AA2++){
					...
					numRotForAAatPos2 = strandRot[str2].rl.getNumRotForAAtype(curAA2);
					if (numRotForAAatPos2==0)	//ala or gly
						numRotForAAatPos2 = 1;
