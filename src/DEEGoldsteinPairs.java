...
public class DEEGoldsteinPairs {
    ...
    // determines which two residues are in the current pair (only for the
    // distributed DEE)
    ...
    // constructor
    DEEGoldsteinPairs(float arpMatrix[][][][][][],
	    float arpMatrixMax[][][][][][], int numResMutable, int strMut[][],
	    float initEw, StrandRotamers strandLRot[],
	    PrunedRotamers<Boolean> prunedRotAtRes, boolean residueMut[],
	    boolean doMin, boolean spFlags[][][][][][], boolean useSF,
	    boolean mb, boolean dDEE, boolean minBB, boolean scaleInt,
	    float maxSc, int mutRes2StrandP[], int mutRes2MutIndexP[],
	...
	if (doMinimize && !doIMinDEE) { // compute the DE pairs MinDEE interval
					// terms
		...
		for (int posNum2 = posNum1 + 1; posNum2 < numRes; posNum2++) {
		    ...
		    indIntMinDEE2Pos[posNum1][posNum2] = SumMaxIndInt(posNum1,
			    posNum2); // formula term 3
		    pairIntMinDEE2Pos[posNum1][posNum2] = SumSumMaxPairInt(
			    posNum1, posNum2); // formula term 4
		    indIntMinDEE2Pos[posNum2][posNum1] = indIntMinDEE2Pos[posNum1][posNum2]; // formula
											     // term
											     // 3
		    pairIntMinDEE2Pos[posNum2][posNum1] = pairIntMinDEE2Pos[posNum1][posNum2]; // formula
											       // term
											       // 4
		}
	    }
	...
	while (!done) {
		...
		if ((magicBullet) || (!distrDEE) || (resInPair[curPos1])) { // mb-pairs
									    // or
									    // not
									    // distrDEE
									    // or
			...
			if ((magicBullet) || (!distrDEE)
				|| (resInPair[curPos2])) { // mb-pairs or not
							   // distrDEE or cur
							   // res is in distr
							   // pair
				...
				// find how many rotamers are allowed for the
				// current AA type at the given residue;
				// note that ala and gly have 0 possible
				// rotamers
				numRotForCurAAatPos1 = strandRot[str1].rl
				...
				for (int curRot1 = 0; curRot1 < numRotForCurAAatPos1; curRot1++) {
				    ...
				    // int index_1 = curPos1*numTotalRot +
				    // rotIndOffset[curAA1] + curRot1;

				    if (!eliminatedRotAtPos.get(curPos1,
					    curAA1, curRot1)) {// not already
							       // pruned
					...
					for (int AA2 = 0; AA2 < numAAtypes[curPos2]; AA2++) {
						...
						if (!eliminatedRotAtPos.get(
							curPos2, curAA2,
							curRot2)) {// not
								   // already
								   // pruned

						    if (!splitFlags[curPos1][curAA1][curRot1][curPos2][curAA2][curRot2]) { // rotamer
															   // pair
															   // not
															   // already
															   // pruned
	     ...
	     * if (numLigRot!=0){
	     * 
	     * if ((magicBullet)||(!distrDEE)||(resInPair[numSiteResidues])){
	     * //mb-pairs or not distrDEE or ligand is in distr pair
	     * 
	     * System.out.print("Starting ligand run"); System.out.print("..");
	     * 
	     * for (int curPos2=0; curPos2<numSiteResidues; curPos2++){
	     * //curPos2 is AS residue, always != ligand
	     * 
	     * if ((magicBullet)||(!distrDEE)||(resInPair[curPos2])){ //mb-pairs
	     * or not distrDEE or cur res is in distr pair
	     * 
	     * for (int curRotLig=0; curRotLig<numLigRot; curRotLig++){ int
	     * indexLig = numSiteResidues*numTotalRot+curRotLig;
	     * 
	     * if (!eliminatedRotAtPos[indexLig]){//not already pruned
	     * 
	     * for (int AA2=0; AA2<numAAtypes[curPos2]; AA2++){ int curAA2 =
	     * sysLR.getIndexOfNthAllowable(residueMap[curPos2],AA2); int
	     * numRotForCurAAatPos2 = rl.getNumRotForAAtype(curAA2); if
	     * (numRotForCurAAatPos2==0) //ala or gly numRotForCurAAatPos2 = 1;
	     * 
	     * for(int curRot2=0; curRot2<numRotForCurAAatPos2; curRot2++){
	     * 
	     * int index_2 = curPos2*numTotalRot + rotIndOffset[curAA2] +
	     * curRot2;
	     * 
	     * if (!eliminatedRotAtPos[index_2]){//not already pruned
	     * 
	     * if (!splitFlags[indexLig][index_2]){ //rotamer pair not already
	     * pruned
	     * 
	     ...
	     * 
	     * prunedCurRun++; } } } } } } } } } System.out.println("done"); } }
	     */
	    ...
	    if ((!magicBullet) && (!distrDEE)) // full non-distributed pairs, so
					       // do not repeat
    ...
    private boolean CanEliminate(int posNum1, int AANumAtPos1,
	    int rotNumAtPos1, int posNum2, int AANumAtPos2, int rotNumAtPos2) {
	...
	// in column 0)
	// index_r1 = posNum1*numTotalRot + rotIndOffset[AANumAtPos1] +
	// rotNumAtPos1;
	// index_r2 = posNum2*numTotalRot + rotIndOffset[AANumAtPos2] +
	// rotNumAtPos2;
	minIndVoxelE = pairwiseMinEnergyMatrix[posNum1][AANumAtPos1][rotNumAtPos1][posNum1][0][0]
		+ pairwiseMinEnergyMatrix[posNum2][AANumAtPos2][rotNumAtPos2][posNum2][0][0]; // formula
											      // term
											      // 1
	minShellResE = RotamerSearch.getShellRotE(pairwiseMinEnergyMatrix,
		posNum1, AANumAtPos1, rotNumAtPos1)
		+ RotamerSearch.getShellRotE(pairwiseMinEnergyMatrix, posNum2,
			AANumAtPos2, rotNumAtPos2);// pairwiseMinEnergyMatrix[posNum1][AANumAtPos1][rotNumAtPos1][posNum1][0][1]
						   // +
						   // pairwiseMinEnergyMatrix[posNum2][AANumAtPos2][rotNumAtPos2][posNum2][0][1];
	minPairE = pairwiseMinEnergyMatrix[posNum1][AANumAtPos1][rotNumAtPos1][posNum2][AANumAtPos2][rotNumAtPos2];

	// if
	// ((minIndVoxelE<=stericEThreshIntra)&&(minShellResE<=stericEThreshPair)){//check
	...
	if ((!eliminatedRotAtPos.get(posNum1, AANumAtPos1, rotNumAtPos1))
		...
		&& (!splitFlags[posNum1][AANumAtPos1][rotNumAtPos1][posNum2][AANumAtPos2][rotNumAtPos2])) { // not
													    // pruned

	    // 2010: Don't compute the intervals if useMinDEEPruningEw is true
	    // since they are not neces
	    if (doMinimize && !doIMinDEE) { // MinDEE, so compute the interval
					    // terms
		indVoxelInterval = indIntMinDEE2Pos[posNum1][posNum2]; // formula
								       // term 3
		pairVoxelInterval = pairIntMinDEE2Pos[posNum1][posNum2]; // formula
									 // term
									 // 4
		...
		int altAA1 = strandRot[str1].getIndexOfNthAllowable(strResNum1,
			AA1);

		numRotForAAatPos1 = strandRot[str1].rl
			.getNumRotForAAtype(altAA1);
		if (numRotForAAatPos1 == 0) // ala or gly
		    numRotForAAatPos1 = 1;

		for (int altRot1 = 0; altRot1 < numRotForAAatPos1; altRot1++) {

		    // if t and r are not actually the same rotamer of the same
		    // AA
		    if (!((altAA1 == AANumAtPos1) && (altRot1 == rotNumAtPos1))) {

			// index_t1 = posNum1*numTotalRot + rotIndOffset[altAA1]
			// + altRot1;

			if ((!eliminatedRotAtPos.get(posNum1, altAA1, altRot1))) { // not
										   // pruned
				...
				int altAA2 = strandRot[str2]
					.getIndexOfNthAllowable(strResNum2, AA2);
				if (!typeDependent
					|| (altAA1 == AANumAtPos1 && altAA2 == AANumAtPos2)) {
				    numRotForAAatPos2 = strandRot[str2].rl
					    .getNumRotForAAtype(altAA2);
				    if (numRotForAAatPos2 == 0) // ala or gly
					numRotForAAatPos2 = 1;

				    for (int altRot2 = 0; altRot2 < numRotForAAatPos2; altRot2++) {

					// if t and r are not actually the same
					// rotamer of the same AA
					if (!((altAA2 == AANumAtPos2) && (altRot2 == rotNumAtPos2))) {

					    // index_t2 = posNum2*numTotalRot +
					    // rotIndOffset[altAA2] + altRot2;

					    if ((!eliminatedRotAtPos.get(
						    posNum2, altAA2, altRot2))) { // not
										  // pruned

						maxIndVoxelE = pairwiseMaxEnergyMatrix[posNum1][altAA1][altRot1][posNum1][0][0]
							+ pairwiseMaxEnergyMatrix[posNum2][altAA2][altRot2][posNum2][0][0]; // formula
															    // term
															    // 2
						maxShellResE = RotamerSearch
							.getShellRotE(
							...
							+ RotamerSearch
								.getShellRotE(
									...
									posNum2,
									altAA2,
									altRot2);// pairwiseMaxEnergyMatrix[posNum1][altAA1][altRot1][posNum1][0][1]
										 // +
										 // pairwiseMaxEnergyMatrix[posNum2][altAA2][altRot2][posNum2][0][1];
						...
						checkSum = -templateInt
							+ (minIndVoxelE
								+ minShellResE + minPairE)
							- (maxIndVoxelE
								+ maxShellResE + maxPairE)
							- indVoxelInterval
							- pairVoxelInterval
							+ minDiffPairVoxelE;
						...
						if (checkSum > curEw) {
						 ...
						 // pruned/eliminated
						    ...
						    minDiff = Math.max(minDiff,
							    checkSum);

						    if (magicBullet) // magic
								     // bullet
								     // pairs,
								     // so no
								     // further
    ...
    private double SumMinDiffPVE(int atPos1, int withAA1, int withRot1,
	    int altAA1, int altRot1, int atPos2, int withAA2, int withRot2,
	    int altAA2, int altRot2) {
	...
	for (int curPos = 0; curPos < numMutable; curPos++) {
		...
		sum += IndMinDiffPVE(atPos1, withAA1, withRot1, altAA1,
			altRot1, atPos2, withAA2, withRot2, altAA2, altRot2,
			curPos);
	}
	 ...
	 * (atPos1, withAA1, withRot1, altAA1, altRot1, atPos2, withAA2,
	 * withRot2, altAA2, altRot2); }
	 */
    ...
    private double IndMinDiffPVE(int firstPos, int firstAA1, int firstRot1,
	    int firstAltAA1, int firstAltRot1, int secondPos, int secondAA1,
	    int secondRot1, int secondAltAA1, int secondAltRot1, int thirdPos) {
	...
	// r at i
	index_r1 = new Index3(firstPos, firstAA1, firstRot1);// firstPos*numTotalRot
							     // +
							     // rotIndOffset[firstAA1]
							     // + firstRot1;
	index_r2 = new Index3(secondPos, secondAA1, secondRot1);// secondPos*numTotalRot
								// +
								// rotIndOffset[secondAA1]
								// + secondRot1;

	// t at i
	index_t1 = new Index3(firstPos, firstAltAA1, firstAltRot1);// firstPos*numTotalRot
								   // +
								   // rotIndOffset[firstAltAA1]
								   // +
								   // firstAltRot1;
	index_t2 = new Index3(secondPos, secondAltAA1, secondAltRot1);// secondPos*numTotalRot
								      // +
								      // rotIndOffset[secondAltAA1]
								      // +
								      // secondAltRot1;
	...
	if (((!eliminatedRotAtPos.get(index_r1)) && (!eliminatedRotAtPos
		.get(index_r2)))
		&& ((!eliminatedRotAtPos.get(index_t1)) && (!eliminatedRotAtPos
			.get(index_t2)))) { // not pruned

	    for (int AA = 0; AA < numAAtypes[thirdPos]; AA++) {

		int curAA = strandRot[str3].getIndexOfNthAllowable(strResNum3,
			AA);

		numRotForAAatPos = strandRot[str3].rl.getNumRotForAAtype(curAA);
		...
		for (int curRot = 0; curRot < numRotForAAatPos; curRot++) {
		    ...
		    // s at j
		    index2 = new Index3(thirdPos, curAA, curRot);// thirdPos*numTotalRot
								 // +
								 // rotIndOffset[curAA]
								 // + curRot;
			...
			if ((!useFlags)
				|| ((!splitFlags[firstPos][firstAA1][firstRot1][thirdPos][curAA][curRot]) && (!splitFlags[secondPos][secondAA1][secondRot1][thirdPos][curAA][curRot]))) { // not
																							  // using
																							  // split
																							  // flags
																							  // or
			    ...
			    curEmin = pairwiseMinEnergyMatrix[firstPos][firstAA1][firstRot1][thirdPos][curAA][curRot]
				    + pairwiseMinEnergyMatrix[secondPos][secondAA1][secondRot1][thirdPos][curAA][curRot];
			    curEmax = pairwiseMaxEnergyMatrix[firstPos][firstAltAA1][firstAltRot1][thirdPos][curAA][curRot]
				    + pairwiseMaxEnergyMatrix[secondPos][secondAltAA1][secondAltRot1][thirdPos][curAA][curRot];
     ...
     * firstRot1, int firstAltAA1, int firstAltRot1, int secondPos, int
     * secondAA1, int secondRot1, int secondAltAA1, int secondAltRot1){
     * 
     * double minE = bigE; double curEmin, curEmax;
     * 
     * int index_r1, index_r2, index_t1, index_t2, index2;
     * 
     * //r at i index_r1 = firstPos*numTotalRot + rotIndOffset[firstAA1] +
     * firstRot1; index_r2 = secondPos*numTotalRot + rotIndOffset[secondAA1] +
     * secondRot1;
     * 
     * //t at i index_t1 = firstPos*numTotalRot + rotIndOffset[firstAltAA1] +
     * firstAltRot1; index_t2 = secondPos*numTotalRot +
     * rotIndOffset[secondAltAA1] + secondAltRot1;
     * 
     * boolean found = false;
     * 
     * if (((!eliminatedRotAtPos[index_r1])&&(!eliminatedRotAtPos[index_r2]))&&
     * ((!eliminatedRotAtPos[index_t1])&&(!eliminatedRotAtPos[index_t2]))){
     * //not pruned
     * 
     ...
     * 
     * if
     * ((!useFlags)||((!splitFlags[index_r1][index2])&&(!splitFlags[index_r2][
     * index2]))){ //not using split flags or not flagged
     * 
     * curEmin =
     * pairwiseMinEnergyMatrix[firstPos][firstAA1][firstRot1][numSiteResidues
     * ][ligAANum][curLigPos] +
     * pairwiseMinEnergyMatrix[secondPos][secondAA1][secondRot1
     * ][numSiteResidues][ligAANum][curLigPos]; curEmax =
     * pairwiseMaxEnergyMatrix
     * [firstPos][firstAltAA1][firstAltRot1][numSiteResidues
     * ][ligAANum][curLigPos] +
     * pairwiseMaxEnergyMatrix[secondPos][secondAltAA1][
     * secondAltRot1][numSiteResidues][ligAANum][curLigPos];
     * 
     * if ((curEmin-curEmax) < minE) minE = curEmin-curEmax;
     * 
     * found = true; } } }
     * 
	 ...
	 * if (numLigRot!=0){ //there is a ligand //get the contribution from
	 * the ligand rotamers: there is only one ligand residue, //so there is
	 * only one position j here for which to add if
	 * ((withoutPos1!=numSiteResidues)&&(withoutPos2!=numSiteResidues)) //if
	 * we are not currently checking ligand rotamers for pruning sum +=
	 * maxScale*LigandMaxIndInt(); }
	 */
	...
	double maxEInt = -999999.0;// this is OK, since E intervals are always
				   // positive
	double curEInt;
	...
	// s at j
	Index3 index1 = new Index3(atPos, atAA, atRot);// atPos*numTotalRot +
						       // rotIndOffset[atAA] +
						       // atRot;
	...
	if ((!eliminatedRotAtPos.get(index1))) { // not pruned
	    ...
	    double maxShell = RotamerSearch.getShellRotE(
		    pairwiseMaxEnergyMatrix, atPos, atAA, atRot);// pairwiseMaxEnergyMatrix[atPos][atAA][atRot][atPos][0][1];
     ...
     * 
     * double maxEInt = -999999.0;//this is OK, since E intervals are always
     * positive double curEInt;
     * 
     * for (int curLigPos=0; curLigPos<numLigRot; curLigPos++){
     * 
     * curEInt = LigandIndInt(curLigPos);
     * 
     * if (curEInt > maxEInt){ maxEInt = curEInt; } }
     * 
     * return maxEInt; }
     */
     ...
     * 
     * double maxE =
     * pairwiseMaxEnergyMatrix[numSiteResidues][ligAANum][ligRot][numSiteResidues
     * ][0][0]; double minE =
     * pairwiseMinEnergyMatrix[numSiteResidues][ligAANum][
     * ligRot][numSiteResidues][0][0];
     * 
     * double maxShell =
     * pairwiseMaxEnergyMatrix[numSiteResidues][ligAANum][ligRot
     * ][numSiteResidues][0][1]; double minShell =
     * pairwiseMinEnergyMatrix[numSiteResidues
     * ][ligAANum][ligRot][numSiteResidues][0][1];
     * 
     * //if ((maxE<=stericEThreshIntra)&&(minE<=stericEThreshIntra) //
     * &&(maxShell<=stericEThreshPair)&&(minShell<=stericEThreshPair)) return
     * ((maxE+maxShell) - (minE+minShell)); } else return 0.0;//contributes
     * nothing }
     */
    // ////////////////////////////////////////////////////////////////////////////////
	 ...
	 * withoutPos1)&&(curPos != withoutPos2)){
	 * 
	...
	for (int AA1 = 0; AA1 < numAAtypes[atPos1]; AA1++) {
		...
		for (int AA2 = 0; AA2 < numAAtypes[atPos2]; AA2++) {
			...
			curEInt = PairInt(atPos1, curAA1, curRot1, atPos2,
				curAA2, curRot2);
			if (curEInt > maxEInt) {
			    maxEInt = curEInt;
			}
    ...
    private double PairInt(int atPos1, int atAA1, int atRot1, int atPos2,
     ...
     * for (int curLigPos=0; curLigPos<numLigRot; curLigPos++){
     * 
     * curEInt = LigandPairInt(atPos, curAA, curRot, curLigPos); if (curEInt >
     * maxEInt){ maxEInt = curEInt; } } } }
     * 
     * return maxEInt; }
     */
    ...
    /*
     * private double LigandPairInt (int atPos, int atAA, int atRot, int
     * ligRot){
     * 
     * //There is a displacement: colum 0 and row 0 have special entries, //so
     * pairwise energies start from row 1, column 1 int index1 =
     * numSiteResidues*numTotalRot + ligRot;//u at k (the ligand residue) int
     * index2 = atPos*numTotalRot + rotIndOffset[atAA] + atRot;//s at j
     * 
     * if (((!eliminatedRotAtPos[index1]))&& ((!eliminatedRotAtPos[index2]))){
     * //not pruned
     * 
     * double maxE =
     * pairwiseMaxEnergyMatrix[numSiteResidues][ligAANum][ligRot][atPos
     * ][atAA][atRot]; double minE =
     * pairwiseMinEnergyMatrix[numSiteResidues][ligAANum
     * ][ligRot][atPos][atAA][atRot];
     * 
     * //if ((maxE<=stericEThreshPair)&&(minE<=stericEThreshPair)) return (maxE
     * - minE); } else return 0.0;//contributes nothing }
     */
    // /////////////////////////////////////////////////////////////////////////////////////
    ...
    /*
     * private boolean CanEliminateLig (int curLigRot,int posNum2, int
     * AANumAtPos2, int rotNumAtPos2){
     * 
     * double minIndVoxelE, maxIndVoxelE; double minShellResE, maxShellResE;
     * double minPairE, maxPairE; double indVoxelInterval, pairVoxelInterval;
     * double minDiffPairVoxelE;
     * 
     * int index_r1, index_r2, index_t1, index_t2;
     * 
     ...
     * all in column 0) index_r1 = numSiteResidues*numTotalRot + curLigRot;
     * index_r2 = posNum2*numTotalRot + rotIndOffset[AANumAtPos2] +
     * rotNumAtPos2; minIndVoxelE =
     * pairwiseMinEnergyMatrix[numSiteResidues][ligAANum
     * ][curLigRot][numSiteResidues][0][0] +
     * pairwiseMinEnergyMatrix[posNum2][AANumAtPos2
     * ][rotNumAtPos2][posNum2][0][0]; //formula term 1 minShellResE =
     * pairwiseMinEnergyMatrix
     * [numSiteResidues][ligAANum][curLigRot][numSiteResidues][0][1] +
     * pairwiseMinEnergyMatrix
     * [posNum2][AANumAtPos2][rotNumAtPos2][posNum2][0][1]; minPairE =
     * pairwiseMinEnergyMatrix
     * [numSiteResidues][ligAANum][curLigRot][posNum2][AANumAtPos2
     * ][rotNumAtPos2];
     * 
     * //if
     * ((minIndVoxelE<=stericEThreshIntra)&&(minShellResE<=stericEThreshPair
     * )){//check only if not an unallowed steric if
     * ((!eliminatedRotAtPos[index_r1])&&(!eliminatedRotAtPos[index_r2])
     * &&(!splitFlags[index_r1][index_r2])){ //not pruned
     * 
     * if (doMinimize){ //MinDEE, so compute the interval terms indVoxelInterval
     * = indIntMinDEE2Pos[numSiteResidues][posNum2]; //formula term 3
     * pairVoxelInterval = pairIntMinDEE2Pos[numSiteResidues][posNum2];
     * //formula term 4 } else { //traditional-DEE, so no interval terms
     * indVoxelInterval = 0.0; pairVoxelInterval = 0.0; }
     * 
     ...
     * 
     * numRotForAAatPos2 = rl.getNumRotForAAtype(altAA2); if
     * (numRotForAAatPos2==0) //ala or gly numRotForAAatPos2 = 1;
     * 
     * for (int altRot2=0; altRot2<numRotForAAatPos2; altRot2++){
     * 
     * //if t and r are not actually the same rotamer of the same AA if
     * (!((altAA2==AANumAtPos2)&&(altRot2==rotNumAtPos2))){
     * 
     ...
     * numSiteResidues][0][0] +
     * pairwiseMaxEnergyMatrix[posNum2][altAA2][altRot2][posNum2][0][0];
     * //formula term 2 maxShellResE =
     * pairwiseMaxEnergyMatrix[numSiteResidues][ligAANum
     * ][altRot][numSiteResidues][0][1] +
     * pairwiseMaxEnergyMatrix[posNum2][altAA2][altRot2][posNum2][0][1];
     * maxPairE =
     * pairwiseMaxEnergyMatrix[numSiteResidues][ligAANum][altRot][posNum2
     * ][altAA2][altRot2];
     * 
     * minDiffPairVoxelE = SumMinDiffPVELig(curLigRot, altRot, posNum2,
     * AANumAtPos2, rotNumAtPos2, altAA2, altRot2); //formula term 5
     * 
     * checkSum = -templateInt + (minIndVoxelE + minShellResE + minPairE) -
     * (maxIndVoxelE + maxShellResE + maxPairE) - indVoxelInterval -
     * pairVoxelInterval + minDiffPairVoxelE;
     * 
     * if (checkSum > curEw){
     ...
     * Math.max(minDiff,checkSum);
     * 
     * if (magicBullet) //magic bullet pairs, so no further checks return false;
     * } } } } } } } } }
     * 
     * //We have tried all of the other rotamers at the current position and
    ...
    /*
     * private double SumMinDiffPVELig (int withRot1, int altRot1, int atPos2,
     * int withAA2, int withRot2, int altAA2, int altRot2){
     * 
     * double sum = 0;
     * 
     ...
     * if ((curPos!=atPos2))
     * 
     * sum += IndMinDiffPVELig(withRot1, altRot1, atPos2, withAA2, withRot2,
     * altAA2, altRot2, curPos); }
     * 
     * return sum; }
     */
    ...
    /*
     * private double IndMinDiffPVELig (int firstRot1, int firstAltRot1, int
     * secondPos, int secondAA1, int secondRot1, int secondAltAA1, int
     * secondAltRot1, int thirdPos){
     * 
     * double minE = bigE; double curEmin, curEmax;
     * 
     * int index_r1, index_r2, index_t1, index_t2, index2; int numRotForAAatPos;
     * 
     * //r at i index_r1 = numSiteResidues*numTotalRot + firstRot1; index_r2 =
     * secondPos*numTotalRot + rotIndOffset[secondAA1] + secondRot1;
     * 
     * //t at i index_t1 = numSiteResidues*numTotalRot + firstAltRot1; index_t2
     * = secondPos*numTotalRot + rotIndOffset[secondAltAA1] + secondAltRot1;
     * 
     * boolean found = false;
     * 
     * if (((!eliminatedRotAtPos[index_r1])&&(!eliminatedRotAtPos[index_r2]))&&
     ...
     * 
     * //s at j index2 = thirdPos*numTotalRot + rotIndOffset[curAA] + curRot;
     * 
     * if ((!eliminatedRotAtPos[index2])){ //not pruned
     * 
     * if
     ...
     * 
     * curEmin =
     * pairwiseMinEnergyMatrix[numSiteResidues][ligAANum][firstRot1][thirdPos
     * ][curAA][curRot] +
     * pairwiseMinEnergyMatrix[secondPos][secondAA1][secondRot1
     * ][thirdPos][curAA][curRot]; curEmax =
     * pairwiseMaxEnergyMatrix[numSiteResidues
     * ][ligAANum][firstAltRot1][thirdPos][curAA][curRot] +
     * pairwiseMaxEnergyMatrix
     * [secondPos][secondAltAA1][secondAltRot1][thirdPos][curAA][curRot];
     * 
     * if ((curEmin-curEmax) < minE) minE = curEmin-curEmax;
     * 
     * found = true; } } } }
     ...
     * //if(minE==bigE)//make it contribute nothing to the sum // minE = 0; }
     * 
     * if (!found) //no possible pairs found minE = 0.0; //contributes nothing
     * to the sum
     * 
     * return minE; }
     */
