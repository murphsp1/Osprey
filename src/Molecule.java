...
/**
 * Handles functions and data associated with molecules. Handles
 * rotations/translations of (parts of) molecules. Manages the data associated
 * with a molecule; handles changes to the molecule (e.g., coordinate changes,
 * deletion or addition of residues, etc.). Determines the bond information for
 * the molecule.
 * 
 */
public class Molecule implements Serializable {

	// All adding and subtracting of strands, residues, and atoms are
	// "top down".
	...
	Atom origAtom[] = null; // Array of atoms to store a copy of the molecule
	// atoms
	...
	boolean connectivity12Valid = false; // Are connected and connected12 up to
	// date
	int connected[][] = null; // 2D array of connected atoms
	int connected12[] = null; // 1D array of atoms directly connected
	int connected13[] = null; // 1D array of atoms 1-3 connected (1 atom
	// between)
	int connected14[] = null; // 1D array of atoms 1-4 connected (2 atoms
	// between)
	...
	double gradient[] = null; // The computed gradient for this molecule (x,y,z)
	// for each atom
	...
	public int checkMolecule(int level, int vlevel) {
		...
		if (level == 1) {
			...
			if (vlevel >= 2) {
				System.out.println();
				System.out
				.println("Checking molecule based numbering of Strands, Residues, and Atoms...");
			}
			...
			for (int i = 0; i < numberOfStrands; i++) {
				...
				for (int j = 0; j < strand[i].numberOfResidues; j++) {
					if (strand[i].residue[j].moleculeResidueNumber != ++lastRnum) {
						if (vlevel >= 1)
							System.out.println("  Residue " + j + " in strand "
									+ i + " is misnumbered");
						correct = false;
					}
					for (int k = 0; k < strand[i].residue[j].numberOfAtoms; k++) {
						if (strand[i].residue[j].atom[k].moleculeAtomNumber != ++lastAnum) {
							if (vlevel >= 1)
								System.out.println("  Atom " + k
										+ " in residue " + j + " in strand "
										+ i + " is misnumbered");
							correct = false;
						}
					}
			...
			if (correct) {
				if (vlevel >= 2)
					System.out
					.println("  Strand, Residue, and Atom -- molecule relative numbers are correct");
			} else {
				if (vlevel >= 1)
					System.out
					.println("  *** Strand, Residue, and Atom -- molecule relative numbers have errors");
			}
			if (vlevel >= 2)
				System.out
				.println("Checking strand based numbering of Residues...");
			...
			if (correct) {
				if (vlevel >= 2)
					System.out
					.println("  Residue -- strand relative numbers are correct");
			} else {
				if (vlevel >= 1)
					System.out
					.println("  *** Residue -- strand relative numbers have errors");
			}

			if (vlevel >= 2)
				System.out
				.println("Checking residue based numbering of Atoms...");
			...
			if (correct) {
				if (vlevel >= 2)
					System.out
					.println("  Atom -- residue relative numbers are correct");
			} else {
				if (vlevel >= 1)
					System.out
					.println("  *** Atom -- residue relative numbers have errors");
			}
				...
				for (int j = 0; j < strand[i].numberOfResidues; j++) {
					for (int k = 0; k < strand[i].residue[j].numberOfAtoms; k++) {
						if (strand[i].residue[j].atom[k].strandNumber != i) {
							if (vlevel >= 1)
								System.out.println("  Atom " + k
										+ " in residue " + j + " of strand "
										+ i + " has misnumbered .strandNumber");
							correct = false;
						}
					}
				}
						...
						if (strand[i].residue[j].atom[k].strandResidueNumber != j) {
							if (vlevel >= 1)
								...
								.println("  Atom "
										...
										+ " of strand "
										+ i
										+ " has misnumbered .strandResidueNumber");
							correct = false;
						}
					}
				}
			...
			if (correct) {
				if (vlevel >= 2)
					System.out
					.println("  atom.strandResidueNumbers are correct");
			} else {
				if (vlevel >= 1)
					System.out
					.println("  *** atom.strandResidueNumbers have errors");
			}
					...
					if (residue[i].atom[j].moleculeResidueNumber != i) {
						if (vlevel >= 1)
							...
							.println("  Atom "
									...
									+ " in residue "
									+ i
									+ " of molecule has misnumbered .moleculeResidueNumber");
						correct = false;
					}
				}
			}
			if (correct) {
				if (vlevel >= 2)
					System.out
					.println("  atom.moleculeResidueNumbers are correct");
			} else {
				if (vlevel >= 1)
					System.out
					.println("  *** atom.moleculeResidueNumbers have errors");
			}
			...
			for (int i = 0; i < numberOfStrands; i++) {
				for (int j = 0; j < strand[i].numberOfResidues; j++) {
					if (strand[i].residue[j].strandNumber != i) {
						if (vlevel >= 1)
							System.out.println("  Residue " + j + " of strand "
									+ i + " has misnumbered .strandNumber");
						correct = false;
					}
				}
			...
			if (correct) {
				...
				if (vlevel >= 1)
					System.out
					.println("  *** residue.strandNumbers have errors");
			}
				...
				for (int j = 0; j < strand[i].numberOfResidues; j++) {
					if (strand[i].residue[j].numberOfAtoms > 1) {
						if (strand[i].residue[j].atom.length != strand[i].residue[j].numberOfAtoms) {
							if (vlevel >= 1)
								System.out
								.println("  Residue " + j + " of strand " + i + " has mismatched residue.atom.length residue.numberOfAtoms");
							correct = false;
						}
					}
				}
			...
			if (correct) {
				...
				if (vlevel >= 1)
					System.out
					.println("  *** residue.numberOfAtoms have errors");
			}
				...
				if (strand[i].numberOfResidues > 1) {
					if (strand[i].residue.length != strand[i].numberOfResidues) {
						if (vlevel >= 1)
							...
							.println("  Strand "
									+ i
									+ " has mismatched strand.residue.length strand.numberOfResidues");
						correct = false;
					}
				}
			}
			if (correct) {
				...
				if (vlevel >= 1)
					System.out
					.println("  *** strand.numberOfResiues have errors");
			}
			...
			for (int i = 0; i < numberOfStrands; i++) {
				...
				if (atCnt != strand[i].numberOfAtoms) {
					if (vlevel >= 1)
						...
						.println("  Strand "
								+ i
								+ " has a strand.numberOfAtoms that doesn't match with its subresidues");
					correct = false;
				}
			}
			if (correct) {
				...
				if (vlevel >= 1)
					System.out
					.println("  *** strand.numberOfAtoms have errors");
			}
			...
			for (int i = 0; i < numberOfAtoms; i++) {
				if (atom[i].bond == null) {
					if (atom[i].numberOfBonds != 0) {
						if (vlevel >= 1)
							System.out
							.println("  atomMoleculeNumber "
									+ i
									+ " has a bond array with size different than numberOfBonds");
						correct2 = false;
					}
				} else {
				...
				for (int j = 0; j < atom[i].numberOfBonds; j++) {
					if (atom[atom[i].bond[j]].strandNumber != atom[i].strandNumber) {
						if (vlevel >= 1)
							System.out.println("  atomMoleculeNumber " + i
									+ " has a bond outside its strand");
						correct3 = false;
					}
				}
			}
			...
			if (correct) {
				if (vlevel >= 2)
					System.out
					.println("  Atom numberOfConnections are correct");
			} else {
				if (vlevel >= 1)
					System.out
					.println("  *** Atom numberOfConnections have errors");
			}
			if (correct2) {
				if (vlevel >= 2)
					System.out
					.println("  Atom bond array sizes agree with number of bonds");
			} else {
				if (vlevel >= 1)
					System.out
					.println("  *** Atom bond array size doesn't agree with number of bonds (single,double,triple bonds)");
			}
			if (correct3) {
				if (vlevel >= 2)
					System.out
					.println("  Atom bonds are all correctly intrastrand");
			} else {
				if (vlevel >= 1)
						...
						for (int k = 0; k < strand[i].residue[j].numberOfAtoms; k++) {
							...
							for (int l = 0; l < at1.numberOfBonds; l++) {
								...
								if (at2res != at1.moleculeResidueNumber) {
									if (!(at1.name.equals("C") || at1.name
											.equals("N"))) {
										result = 1;
										int at2num = atom[at1.bond[l]].moleculeAtomNumber;
										if (debug)
											System.out
											.print("Error found, bond between "
													+ at1.moleculeAtomNumber
													+ " and " + at2num);
										if (fix) {
											deleteBondBetween(
													at1.moleculeAtomNumber,
	...
	// The molecule numbers of all atoms and residues above strandPos,
	// residuePos
	// are updated.
	...
	private void expandMolecule(int numResidues, int numAtoms, int strandPos, int residuePos) {
		...
		if (numResidues == 1) {
			...
			moleculeAtomInsertPoint = theResidue.atom[0].moleculeAtomNumber
					+ theResidue.numberOfAtoms;
		}
		...
		if (moleculeAtomInsertPoint + numAtoms < newAtomNumber)
			System.arraycopy(atom, moleculeAtomInsertPoint, largerAtomArray,
					numAtoms + moleculeAtomInsertPoint, newAtomNumber
					- (moleculeAtomInsertPoint + numAtoms));
		atom = largerAtomArray;

		if (numResidues == 1) {
			Residue largerResidueArray[] = new Residue[numberOfResidues + 1];
			System.arraycopy(residue, 0, largerResidueArray, 0,
					moleculeResInsertPoint);
			if (moleculeResInsertPoint < numberOfResidues)
				System.arraycopy(residue, moleculeResInsertPoint,
						largerResidueArray, moleculeResInsertPoint + 1,
						numberOfResidues - moleculeResInsertPoint);
		...
		if (moleculeAtomInsertPoint + numAtoms < newAtomNumber)
			System.arraycopy(actualCoordinates, moleculeAtomInsertPointx3,
					largerCoordinateArray, numAtoms * 3
					+ moleculeAtomInsertPointx3, newAtomNumberx3
					- (moleculeAtomInsertPointx3 + numAtoms * 3));
	...
	// Adds a new empty strand with name "strandName" to molecule and updates
	// array.
	...
	// 2) determineBonds() is called later and the atoms bonds[] array is not
	// used immediately, or
	...
	// All molecule, strand, residue, and atom fields are appropriately updated;
	// If updatedBonds is true, then the bonds[] array for each atom in
	// newResidue is computed
	public void addResidue(int strandNumber, Residue newResidue,
			boolean updateBonds) {
		...
		expandMolecule(1, newResidue.numberOfAtoms, strandNumber,
				newResPosition - 1);
		...
		for (int i = 0; i < newResidue.numberOfAtoms; i++, q++) {
			...
			qx3 = q * 3;
			actualCoordinates[qx3] = atom[q].coord[0] = newResidue.atom[i].coord[0];
			actualCoordinates[qx3 + 1] = atom[q].coord[1] = newResidue.atom[i].coord[1];
			actualCoordinates[qx3 + 2] = atom[q].coord[2] = newResidue.atom[i].coord[2];
		}
	...
	public int addAtom(int moleculeResidueNumber, Atom newAtom) {
		...
		expandMolecule(0, 1, residue[moleculeResidueNumber].strandNumber,
		...
		// strand.addAtom() does additional bookkeeping and will call
		// residue.addAtom
	...
	// all residues in strand[i] are numbered less than all residues in
	// strand[j]
	public int deleteStrand(int strandNumber) {
		...
		int newAtomNumberx3 = newAtomNumber * 3;
		int newResidueNumber = numberOfResidues
				- strand[strandNumber].numberOfResidues;
		...
		for (int i = 0; i < (numberOfStrands - 1); i++) {
			for (int j = 0; j < strand[i].numberOfResidues; j++) {
				...
				for (int k = 0; k < strand[i].residue[j].numberOfAtoms; k++) {
					...
					// This is ok because there should not be bonds between
					// strands
					if (i >= strandNumber) {
	...
	public int deleteResidue(int moleculeResidueNumber) {
		...
		strand[strandNumber]
				.deleteResidue(residue[moleculeResidueNumber].strandResidueNumber);

		System.arraycopy(residue, 0, smallerResidueArray, 0,
				moleculeResidueNumber);
		if (moleculeResidueNumber < numberOfResidues - 1)
			System.arraycopy(residue, moleculeResidueNumber + 1,
					smallerResidueArray, moleculeResidueNumber, residue.length
					- moleculeResidueNumber - 1);
		residue = smallerResidueArray;
	...
	public int deleteAtom(String atomName, int moleculeResidueNumber) {
		for (int i = 0; i < residue[moleculeResidueNumber].numberOfAtoms; i++)
			if (residue[moleculeResidueNumber].atom[i].name.equals(atomName))
				return (deleteAtom(residue[moleculeResidueNumber].atom[i].moleculeAtomNumber));
		return (0);
	}
	...
	public int deleteAtom(int strandNumber, int strandResidueNumber, int residueAtomNumber) {
		return (deleteAtom(strand[strandNumber].residue[strandResidueNumber].atom[residueAtomNumber].moleculeAtomNumber));
	}
	...
	public int deleteAtom(int moleculeAtomNumber) {
		...
		if (moleculeAtomNumber < newAtomNumber)
			System.arraycopy(actualCoordinates, moleculeAtomNumberx3 + 3,
					smallerCoordinateArray, moleculeAtomNumberx3,
					newAtomNumberx3 - moleculeAtomNumberx3);
	...
	// Goes through each atom and makes sure its moleculeResidueNumber points to
	// the right residue
	...
	public void connectResidues(int residueA, String atomAName, int residueB,
			String atomBName) {
	...
	public void setTorsion(int a1num, int a2num, int a3num, int a4num,
			double torsion, int atomList[], int numberOfElements) {
		setTorsion(a1num, a2num, a3num, a4num, torsion, atomList,
				numberOfElements, true);
	}

	public void changeTorsion(int a1num, int a2num, int a3num, int a4num,
			double deltaTorsion, int atomList[], int numberOfElements) {
		changeTorsion(a1num, a2num, a3num, a4num, deltaTorsion, atomList,
				numberOfElements, true);
	}
	...
	public void setTorsion(int a1num, int a2num, int a3num, int a4num,
			double torsion, int atomList[], int numberOfElements, boolean updateAtoms) {
		...
		/*Sean - this variable appears to never be used or accessed ...
	...
	public double angleAboutYAxis(float coord0, float coord1, float coord2) {
		final double R2D = 57.29577951308232090712;
		//double distance = Math.sqrt(coord0 * coord0 + coord1 * coord1 + coord2 * coord2);
		double xComponent = coord0; /// / distance;
		double zComponent = coord2; // / distance;
		double theta = 0.0;
	...
	// axis where the +x axis is the 0
	// This was not written by me, but I have checked it
	public double angleAboutZAxis(float coord0, float coord1, float coord2) {
		double R2D = 57.29577951308232090712;
		//double distance = Math.sqrt(coord0 * coord0 + coord1 * coord1 + coord2 * coord2);
		double xComponent = coord0;// / distance;
		double yComponent = coord1;// / distance;
		double theta = 0.0;
	...
	public void changeTorsion(int a1num, int a2num, int a3num, int a4num,
		...
		// Aligns dihedral on the +x-axis, modifies the angle then rotates it
		// back
		int at2x3 = a2num * 3;
		int at3x3 = a3num * 3;
		int at4x3 = a4num * 3;
		...
		for (int i = 0, ix3, atx3; i < atomListLength; i++) {
			...
			if ((atomNumber == a1num) || (atomNumber == a2num) || (atomNumber == a3num) || (atomNumber == a4num))
				continue;
			atx3 = atomNumber * 3;
			ix3 = i * 3;
			temporaryCoordinates[ix3 + 3] = actualCoordinates[atx3] - actualCoordinates[at3x3];
			temporaryCoordinates[ix3 + 4] = actualCoordinates[atx3 + 1] - actualCoordinates[at3x3 + 1];
			temporaryCoordinates[ix3 + 5] = actualCoordinates[atx3 + 2] - actualCoordinates[at3x3 + 2];
		}
		// get angle info from atom2
		RotMatrix theRotation = new RotMatrix();
		
		float[] pseudoAtom2Coords = new float[3];
		pseudoAtom2Coords[0] = actualCoordinates[at2x3] - actualCoordinates[at3x3];
		pseudoAtom2Coords[1] = actualCoordinates[at2x3 + 1] - actualCoordinates[at3x3 + 1];
		pseudoAtom2Coords[2] = actualCoordinates[at2x3 + 2] - actualCoordinates[at3x3 + 2];
		...
		*/
		...
		//double test = Atom.angleAboutYAxis();
		//double a2YAngle = pseudoAtom2.angleAboutYAxis();
		double a2YAngle = angleAboutYAxis(pseudoAtom2Coords[0], pseudoAtom2Coords[1], pseudoAtom2Coords[2]);
		//theRotation.yAxisRotate(-a2YAngle, pseudoAtom2.coord, 1);
		theRotation.yAxisRotate(-a2YAngle, pseudoAtom2Coords, 1);
		//double a2ZAngle = pseudoAtom2.angleAboutZAxis();
		double a2ZAngle = angleAboutZAxis(pseudoAtom2Coords[0], pseudoAtom2Coords[1], pseudoAtom2Coords[2]);
		...
		theRotation.yAxisRotate(a2YAngle, temporaryCoordinates, numberOfCoordinates);
		theRotation.translate(actualCoordinates[at3x3],
				actualCoordinates[at3x3 + 1], actualCoordinates[at3x3 + 2],
				temporaryCoordinates, numberOfCoordinates);
	...
	// Returns the center of mass of the given residue (molecule-relative
	// numbering)
	...
	// Computes the geometric center (from the actualCoordinates[]) for residue
	// resNum (molecule-relative numbering)
	...
	public void translateResidue(int resNum, float dx, float dy, float dz) {
	...
	public void translateResidue(int strNum, int resNum, float dx, float dy,
			float dz) {
	...
	public void translateStrand(int strNum, float dx, float dy, float dz) {
	...
	public void rotateStrand(int ligStrNum, float dx, float dy, float dz,
		...
		for (int w = 0; w < theStrand.numberOfResidues; w++) {
			for (int q = 0; q < theStrand.residue[w].numberOfAtoms; q++) {
				...
				actualCoordinates[baseNum] = (float) (tx * rot_mtx[0][0] + ty
						* rot_mtx[0][1] + tz * rot_mtx[0][2] + cx);
				actualCoordinates[baseNum + 1] = (float) (tx * rot_mtx[1][0]
						+ ty * rot_mtx[1][1] + tz * rot_mtx[1][2] + cy);
				actualCoordinates[baseNum + 2] = (float) (tx * rot_mtx[2][0]
						+ ty * rot_mtx[2][1] + tz * rot_mtx[2][2] + cz);
	...
	// Rotates the given residue (molecule-relative numbering) by thetaDeg
	// degrees
	...
	public void rotateResidue(int resNum, float dx, float dy, float dz,
		...
		for (int q = 0; q < residue[resNum].numberOfAtoms; q++) {
			...
			actualCoordinates[baseNum] = (float) (tx * rot_mtx[0][0] + ty
					* rot_mtx[0][1] + tz * rot_mtx[0][2] + cx);
			actualCoordinates[baseNum + 1] = (float) (tx * rot_mtx[1][0] + ty
					* rot_mtx[1][1] + tz * rot_mtx[1][2] + cy);
			actualCoordinates[baseNum + 2] = (float) (tx * rot_mtx[2][0] + ty
	...
	// Rotates the given atomList (molecule-relative numbering) by thetaDeg
	// degrees
	// around axis dx, dy, dz (around the point cs,cy,cz)
	...
	// Rotates the given atom i (molecule-relative numbering) by thetaDeg
	// degrees
	// around axis dx, dy, dz (around the point cs,cy,cz)
	public void rotateAtom(int i, float dx, float dy, float dz, double cx,
			double cy, double cz, float thetaDeg, boolean updateAtoms) {
	...
	public void rotateAroundCOM(double dx, double dy, double dz, double thetaDeg) {

		// Get center of mass
		float[] centOfMass = getCenterOfMass();

		rotateMolecule(dx, dy, dz, thetaDeg, centOfMass[0], centOfMass[1],
				centOfMass[2]);
	}
	...
	// *Note that connectivity calculations check single, double, and triple
	// bonds.
	public void establishConnectivity(boolean just12) {
		...
		System.arraycopy(connected12, 0, smallerConnectArray, 0,
				numberOf12Connections * 2);
		...
		System.arraycopy(connected13, 0, smallerConnectArray, 0,
				numberOf13Connections * 3);
		...
		for (int i = 0; i < numberOf13Connectionsx3; i = i + 3) {
			...
			for (int l = 1; l < connected[atomi][0] + 1; l++) {
				...
				if ((atomx != atomj) && (atomx != atomk)) {
					if (isNOT13Connected(atomx, atomk)
							&& isNOTAlready14Connected(atomx, atomk)) {
			...
			for (int l = 1; l < connected[atomk][0] + 1; l++) {
				...
				if ((atomx != atomi) && (atomx != atomj)) {
					if (isNOT13Connected(atomx, atomi)
							&& isNOTAlready14Connected(atomx, atomi)) {
		...
		System.arraycopy(connected14, 0, smallerConnectArray, 0,
				numberOf14Connections * 4);
			...
			for (int j = i + 1; j < numberOfAtoms; j++) {
				if (bondedMatrix[i][j] == false) {
					...
					if (numberOfNBConnectionsx2 >= nonBonded.length) {
						int largerConnectArray[] = new int[numberOfNBConnectionsx2
						                                   + NB_INCREMENT_SIZE * 2];
						System.arraycopy(nonBonded, 0, largerConnectArray, 0,
								nonBonded.length);
		...
		System.arraycopy(nonBonded, 0, smallerConnectArray, 0,
				numberOfNBConnectionsx2);
	...
	private void connect(int atom1, int atom2) {
		if (debug) {
			if (connected[atom1][0] >= 6) {
				System.out.println("Debug: Error in molecule.connect. Atom "
						+ atom1 + " has too many bonds");
				System.out.println("molecAtNum: "
						+ atom[atom1].moleculeAtomNumber + " resAtNum: "
						+ atom[atom1].residueAtomNumber);
			}
		}
	...
	// NOTE: nonBonded[] contains all pairs of atoms that are more than 3 bonds
	// apart
	...
	// Determines if a bond exists between each pair of atoms in this molecule
	// as a function of the distance (atom.coord[] and not
	// actualCoordinates[]) and atom types;
	// Clears the bond information for all atoms in the molecule first;
	// TODO: for now determining bonds by distance is ok, but we eventually
	// might want to read the CONECT terms if they exist
	public void determineBonds() {
		...
		for (int i = 0; i < numberOfAtoms; i++) {
			for (int j = i + 1; j < numberOfAtoms; j++) {
				...
				if ((atom[i].elementType.equalsIgnoreCase("H") || atom[j].elementType
						.equalsIgnoreCase("H"))
						&& (atom[i].elementType.equalsIgnoreCase("S") || atom[j].elementType
								.equalsIgnoreCase("S"))) {
					if (dist < 1.5) {
				...
				} else if ((atom[i].elementType.equalsIgnoreCase("H") || atom[j].elementType
						.equalsIgnoreCase("H"))
						&& !(atom[i].elementType.equalsIgnoreCase("S") || atom[j].elementType
								.equalsIgnoreCase("S"))) {
					if (dist < 1.2) {
	...
	// Returns the molecule-relative residue number of the residue with PDB
	// residue number pdbResNum
	/*
	 * public int mapPDBresNumToMolResNum(int pdbResNum){ for (int i=0;
	 * i<numberOfResidues; i++){ if (residue[i].getResNumber()==pdbResNum){
	 * return residue[i].moleculeResidueNumber; } } return -1; }
	 */
	...
	public float[] getActualCoords() {
		...
		System.arraycopy(actualCoordinates, 0, retCoord, 0,
				actualCoordinates.length);
		return retCoord;
	}
