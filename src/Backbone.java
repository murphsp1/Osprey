...
/**
 * Handles the backbone representation for the protein; Applies (phi,psi)
 * changes to the molecule; Assumes that the order of the atoms for the phi
 * angle is C(i-1), N(i), CA(i), C(i), and for the psi angle: N(i), CA(i), C(i),
 * N(i+1)
 * 
 */
public class Backbone implements Serializable {
    ...
    // Get the current phi (angleType==0) or psi (angleType=1) value for the
    // given residue resNum of strand strandNum in molecule m
    public double getFiPsi(Molecule m, int strandNum, int resNum, int angleType) {

	if ((angleType == 0)
		&& ((resNum == 0) || (m.strand[strandNum].residue[resNum - 1]
			.getResNumber() != m.strand[strandNum].residue[resNum]
			.getResNumber() - 1))) { // phi angle needed and
						 // (N-terminus or no prev
						 // residue), so no phi angle
	    return 0.0;
	} else if ((angleType == 1)
		&& ((resNum == (m.strand[strandNum].numberOfResidues - 1)) || (m.strand[strandNum].residue[resNum + 1]
			.getResNumber() != m.strand[strandNum].residue[resNum]
			.getResNumber() + 1))) { // psi angle needed and
						 // (C-terminus or no next
						 // residue), so no psi angle
	    return 0.0;
	} else {
	...
	getAtforRes("C", prevRes, at, 3); // first find the C atom of the
					  // previous residue
	getAtforRes("N", curRes, at, 2); // then find the N, CA, and C atoms of
					 // the current residue
	...
	getAtforRes("N", curRes, at, 0); // first find the N, CA, and C atoms of
					 // the current residue
	getAtforRes("CA", curRes, at, 1);
	getAtforRes("C", curRes, at, 2);
	getAtforRes("N", nextRes, at, 3); // then find the N atom of the next
					  // residue
    }
    ...
    public void applyFiPsi(Molecule m, int strandNum, int resNum, double alpha,
	    int angleType) {
	...
	if ((angleType == 0)
	    ...
	    int atomList[] = new int[m.numberOfAtoms]; // list of atoms to be
						       // rotated
		...
		// find the atoms involved in the psi angle
		getPsiAtoms(curRes, nextRes, at);
	    ...
	    double curTorsion = at[3].torsion(at[0], at[1], at[2]); // the
								    // current
								    // torsion
    ...
    // Determines which atoms need to be rotated when the phi or psi change is
    // applied, for a given strand strandNum in molecule m;
    // Only certain atoms for the residue to which atAtom belongs will be
    // rotated;
    // All atoms belonging to the residues before this one will be rotated
    private void getAtomsToRotate(Molecule m, int strandNum, Atom atAtom,
	    int atomList[], int angleType) {
    ...
    // Determines which atoms need to be rotated when the phi or psi change is
    // applied for a given residue
    private void getAtomsToRotateHelper(Molecule m, int curRes, Atom atAtom,
	    int atomList[]) {
