...
/**
 * Handles reading in and managing parameter/value pairs from the input
 * configuration files
 */
public class ParamSet implements Serializable {
    ...
    // constructor
    ParamSet() {
	int initSize = 50; // the initial size of the arrays; increased if
			   // necessary
	...
	try {
		...
		if (curLine != null) {
		    ...
		    if (ind >= 0) { // parameter already read, but only one
				    // definition should exist
			System.out.println("ERROR: parameter " + paramName
				+ " already read");
			System.exit(1);
		    } else { // new parameter
			if (curNum >= params.length) { // double the size of the
						       // arrays
			...
			values[curNum] = curLine.substring(params[curNum]
				.length() + 1);
			curNum++;
			curLine = bufread.readLine();
		    }
	    ...
	    System.out
		    .println("ERROR: An error occurred reading configuration file "
			    + fName);
	    System.exit(1);
	}
    }

    // Returns the index into params[] of the parameter paramName; return -1 if
    // not found
    ...
    // Returns the value from values[] that corresponds to the parameter
    // paramName from params[];
	...
	if (ind >= 0) {
	    if (processRank == 0) {
		System.out.println("Parameter " + paramName + " set to "
			+ values[ind]);
	    }
	    return values[ind];
	} else {
		...
		System.out.println("Parameter " + paramName
			+ " not set. Using default value " + defaultVal);
	    }
	    return defaultVal;
	}
    }
