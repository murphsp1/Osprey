...
/**
 * 
 * This class reads from three data files specifying amino acid residues,
 * N-terminal amino acid residues, and C-terminal amino acid residues.
 * Information read includes element type, atom type, limited connectivity, and
 * partial charge. By matching these amino acid templates to actual residues in
 * a molecule, the corresponding template atom types and partial charges can be
 * assigned to the matched residues.
 * 
 */
public class AminoAcidTemplates implements Serializable {
	...
	while (curLine != null) {
		...
		// KER: The first atom is bonded to a dummy atom so we can't
		// include that
		// KER: in the bond list, so check atom is >= 0
		int atomBondedTo = ((new Integer(getToken(curLine, 5)))
			.intValue()) - dumPresent;
		if (atomBondedTo >= 0)
		...
		if (getToken(curLine, 1).equalsIgnoreCase("LOOP")) {
			...
			for (Atom a : newRes.atom) {
				...
				for (Atom b : newRes.atom) {
				    if (b.name.equalsIgnoreCase(getToken(
					    curLine, 2))) {
	...
	is = new FileInputStream(EnvironmentVars.getDataDir().concat(
		aaCoordsFilename));
	...
	while (curLine != null) {
		...
		if (r.name.equalsIgnoreCase(resName)) {
			...
			for (Atom a : r.atom) {
			    if (a.name.equalsIgnoreCase(getToken(curLine, 1))) {
				foundAtom = true;
				a.setCoords(new Float(getToken(curLine, 2)),
					new Float(getToken(curLine, 3)),
					new Float(getToken(curLine, 4)));
				break;
			    }
			}
			if (!foundAtom) {
			    System.out
				    .println("Residue coord and forcefield templates did not match up.");
			    System.exit(0);
			}
		    }
		    break;
