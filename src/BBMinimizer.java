...
/**
 * Handles two types of backbone energy minimization for BD: (1) the
 * minimization required for computing the pairwise energy matrices; (2) the
 * minimization of a full conformation: the side-chain dihedrals are kept rigid,
...
public class BBMinimizer implements Serializable {
    ...
    // int residueMap[] = null; //the mapping between AS and molecule-relative
    // residue numbering
    int strandMut[][] = null;
    ...
    int numberOfStrands;
    final int numStepsFiPsi = 15; // the number of phi/psi minimization steps to
				  // be performed
    final float sysMaxDihedRot = 3.0f; // the maximum rotation from the initial
				       // phi/sp values
    private float sysFiPsiStep = 0.8f; // initial step size for phi/psi changes;
				       // used for full conformation energy
				       // minimization

    float molCurFiPsiDisp[][] = null; // the current displacement from the
				      // initial phi/psi angle for each residue

    float ligRotSize = 0.5f; // initial rotation angle size (in degrees)
    float ligTransSize = 0.5f; // initial translation size (in angstrom)

    float sysMaxTrans = 1.5f;// 2.0f; //the maximum displacement (in angstrom)
			     // from the initial CA
    private int molAtNumCA[] = null; // the molecule atom numbers for the CA
				     // atoms for each residue
    double molStartCA[][] = null; // the position of the initial CA for each
				  // residue
    private double molCurCA[][] = null; // the current CA coordinates for each
					// residue
	...
	/*
	 * int curNumFlex = 0; Residue localRes = null; for(int
	 * i=0;i<m.strand[sysStrNum].numberOfResidues;i++){ localRes =
	 * m.strand[sysStrNum].residue[i]; if(localRes.flexible){
	 * flexResListSize[curNumFlex] = localRes.numberOfAtoms; for(int
	 * k=0;k<flexResListSize[curNumFlex];k++){
	 * flexResAtomList[curNumFlex][k] = localRes.atom[k].moleculeAtomNumber;
	 * } curNumFlex++; } }
	 */

	molAtNumCA = getMolAtNumCA(); // this must be set before setting
				      // molStartCA and molCurCA
     ...
     * residueMap[], int sysStrand, int ligStrand){
     * 
     ...
     * information initialize(mol,theA96ff,residueMap,sysStrand);
     * 
    ...
    // Minimizes the energy of a fully-assigned AA sequence with fully-assigned
    // rotamer identities,
	...
	for (int i = 0; i < numStepsFiPsi; i++) {
	    for (int str = 0; str < numberOfStrands; str++) {
		for (int j = 0; j < strandMut[str].length; j++) { // apply the
								  // current
								  // phi/psi
								  // step for
								  // each of the
								  // flexible
								  // residues
		    int asResNum = strandMut[str][j]; // strandMut is
						      // strandResidue specific
		    for (int a = 0; a < 2; a++) { // (a==0) applies a phi
						  // change; (a==1) applies a
						  // psi change

			if ((!m.strand[str].residue[asResNum].nterm
				&& !(asResNum == 0) && a == 0)
				|| (!m.strand[str].residue[asResNum].cterm
					&& !(asResNum == (m.strand[str].numberOfResidues - 1)) && a == 1)) {
			    float change = compFiPsi(asResNum, str, a,
				    fiPsiStep, bb, pemComp);
			    if (Math.abs(change) > 0.001) { // smaller changes
							    // should not be
							    // applies
		...
		if (m.strand[str].rotTrans) {
			...
			float dTrans = compTrans(resNums, resNums.length,
				curCoord, transStep, curTransRotInd);
			if (Math.abs(dTrans) != 0.0) {
			    for (int k = 0; k < resNums.length; k++)
				updateCumulativeTrans(resNums[k], curCoord,
					dTrans, false);
			}
		    }
		    for (int curCoord = 0; curCoord < 3; curCoord++) {
			float axisToRot[] = getRotVector(curCoord); // determine
								    // the axis
								    // of
								    // rotation
			...
			for (int j = 0; j < 3; j++)
			    center[j] = myCenter[j];
			float dRot = compRot(resNums, resNums.length, rotStep,
				center, axisToRot, curTransRotInd);
			if (Math.abs(dRot) != 0.0)
			    for (int k = 0; k < resNums.length; k++)
				updateCumulativeRot(resNums[k], dRot, center,
					false, axisToRot, false);
		    }
		}
	    }
    ...
    // Checks the energy if a phi (angleType==0) or psi (angleType==1) change of
    // +fiPsiStep
    // is applied to residue resNum (strand-relative numbering, only residues in
    // strand strandNum are considered);
    // Checks the energy at -fiPsiStep and at the initial position and compares
    // these three energies
    // to determine the optimal direction of change;
    // If at least one residue is moved further than the maximum allowed limit
    // for its CA, then the
    // step in the corresponding direction is not considered
    private float compFiPsi(int resNum, int strandNum, int angleType,
	    float fiPsiStep, Backbone bb, boolean pemComp) {
	...
	if ((!checkCAallRes(strandNum))) // at least one residue moved too far
					 // away or unallowed steric for
					 // PEMcomp, so make this step
					 // impossible
	    secondEnergy[0] = Math.pow(10, 38);
	for (int i = 0; i < m.strand[strandNum].numberOfResidues; i++)
	    restoreCoord(
		    m.strand[strandNum].residue[i].moleculeResidueNumber,
		    storedCoord[m.strand[strandNum].residue[i].moleculeResidueNumber]);
	...
	float step = getDir(initialEnergy[0], secondEnergy[0], thirdEnergy[0],
		fiPsiStep);
    ...
    // Updates the CA coordinates for all residues; should be called from within
    // functions that use/modify molCurCA[]
    ...
    // the CA of resNum further than the limit
    // and applies the optimal translation if (onlyCheck==false)
    private float updateCumulativeTrans(int resNum, int coord, float transStep,
	    boolean onlyCheck) {
	...
	if (dist > sysMaxTrans) { // if the step would take us too far away then
				  // scale it back
	...
	if (!onlyCheck) {// actually apply the new translation
	    ...
	    m.translateResidue(resNum, theTranslation[0], theTranslation[1],
    ...
    private float compRot(int resNums[], int numRes, float rotStep,
	    double center[], float axisToRot[], int AAnum) {
    ...
    private float updateCumulativeRot(int resNum, float rotStep,
	...
	if ((!centerIsCA) && (dist > sysMaxTrans)) { // further than allowed, so
						     // restore, find the
						     // optimal rotStep value,
						     // and rotate with that
						     // value, if necessary

	    restoreCoord(resNum, storedCoord);

	    if (getDist(molStartCA[resNum], molCurCA[resNum]) > (sysMaxTrans - 0.01)) // the
										      // current
										      // CA
										      // is
										      // already
		...
		rotStep = binSearchRotStep(CAcoord, axisToRot, center, rotStep,
			resNum); // find the optimal rotStep value that will
				 // rotate the CA to the max limit
		if ((rotStep != 0.0f) && (!checkOnly)) // apply the rotation
		    m.rotateResidue(resNum, axisToRot[0], axisToRot[1],
			    axisToRot[2], center[0], center[1], center[2],
			    rotStep, false);
	    }
	}

	if (!checkOnly) // rotation is actually performed, so update the CA
			// coordinates
	    molCurCA[resNum] = getCAcoord(resNum);
	else
	    restoreCoord(resNum, storedCoord);
    ...
    private float binSearchRotStep(double coord[], float axisToRot[],
	    double center[], float origTheta, int resNum) {
    ...
    // Returns the molecule atom numbers of the CA atoms for each residue in the
    // molecule;
    // If a given residue does not have a CA atom, the corresponding entry is
    // set to -1
    ...
    // numbering);
    // If the given residue does not have a CA atom, then the coordinates of the
    // geometric center of the residue are returned
	...
	} else { // no CA atom, so compute and return the geometric center of
		 // the residue
    ...
    // Checks the distance between the current CA position and the initial CA
    // position for all residues;
    // If at least one residue has moved too far, return false; otherwise,
    // return true
