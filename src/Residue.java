...
public class Residue implements Serializable {
    ...
    private boolean energyEvalSC = true; // Should the residue side-chain be
					 // included in computing energy
    private boolean energyEvalBB = true; // Should the residue backbone be
					 // included in computing energy
    boolean ffAssigned = false; // Are forcefield types assigned for atoms in
				// this residue
    ...
    // It resets atom bond[] and numberOfBonds, so the bond information must be
    // updated
    ...
    public void rotateResidue(Atom at1, double dx, double dy, double dz,
	    double thetaDeg, int atomList[], int numAtoms) {
	...
	for (int q = 0; q < numAtoms; q++) {
	    qx3 = q * 3;
	    temporaryCoordinates[qx3 + 0] = atom[atomList[q]].coord[0]
		    - at1.coord[0];
	    temporaryCoordinates[qx3 + 1] = atom[atomList[q]].coord[1]
		    - at1.coord[1];
	    temporaryCoordinates[qx3 + 2] = atom[atomList[q]].coord[2]
		    - at1.coord[2];
	}
	...
	for (int q = 0; q < numAtoms; q++) {
	    ...
	    atom[atomList[q]].coord[0] = tx * rot_mtx[0][0] + ty
		    * rot_mtx[0][1] + tz * rot_mtx[0][2] + at1.coord[0];
	    atom[atomList[q]].coord[1] = tx * rot_mtx[1][0] + ty
		    * rot_mtx[1][1] + tz * rot_mtx[1][2] + at1.coord[1];
	    atom[atomList[q]].coord[2] = tx * rot_mtx[2][0] + ty
		    * rot_mtx[2][1] + tz * rot_mtx[2][2] + at1.coord[2];
	}

    }

    // Returns the distance (the minimum distance between a pair of non-hydrogen
    // side-chain atoms) between the side-chains of this residue and res2;
	...
	for (int a1 = 0; a1 < numberOfAtoms; a1++) {
	    ...
	    if ((!a1type.equalsIgnoreCase("H"))
		    && (bbAt || (!atom[a1].getIsBBatom()))) {

		for (int a2 = 0; a2 < res2.numberOfAtoms; a2++) {

		    String a2type = res2.atom[a2].elementType;

		    if ((!a2type.equalsIgnoreCase("H"))
			    && (bbAt || (!res2.atom[a2].getIsBBatom()))) {
    ...
    // Returns the moleculeAtomNumber for the atom of this residue that has the
    // same name as the parameter n
