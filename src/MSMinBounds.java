...
/**
 * Performs two different operations, depending on the input parameters: 1)
 * Applies the Bounds/MinBounds pruning criteria: computes a lower bound on the
 * energy of all conformations that contain a given rotamer i_r, for each
 * rotamer; 2) Computes: (a) a lower bound on the energy of all conformations
 * that contain a pruned rotamer, and (b) all conformations that are pruned due
...
public class MSMinBounds {
    ...
    // int residueMap[] = null;
    int strandMut[][] = null;
    // determines if Ec and prunedIsSteric are computed for the ensemble-based
    // bound to the total
    // contribution of all pruned conformations
    boolean boundKStar = false;

    // determines if bounds are only computed (if true, no pruning or Ec
    // computation is performed)
    boolean onlyBound = false;

    // flag that a rotamer is pruned because of a steric clash, and not because
    ...
    // constructor
    MSMinBounds(float arpMatrix[][][][][][], int numResMutable, int strMut[][],
	    StrandRotamers strRot[], double pruneE,
	    PrunedRotamers<Boolean> prunedRotAtRes,
	    boolean spFlags[][][][][][], boolean useSF, float initEw,
	    boolean boundKS, boolean onlyCompBound, int mutRes2StrandP[],
	    int mutRes2MutIndexP[]) {
	...
	while (iter.hasNext()) {
	    ...
	    // Already Initialized
	    // rotInMutInd[i] = false; //true only if the given rotamer index is
	    // used in the current mutation sequence
	...
	/*
	 * for (int i=0; i<prunedIsSteric.length; i++) prunedIsSteric[i] =
	 * false;
	 */
    ...
    /*
     * Performs two separate operations, depending on the input parameters: 1)
     * Applies the Bounds/MinBounds pruning criteria: computes a lower bound on
     * the energy of all conformations that contain a given rotamer i_r, for
     * each rotamer (boundKStar is false); Return a boolean matrix in which an
     * element is true if the corresponding r at i can be eliminated, and false
     * otherwise. 2) Computes: (a) Ec, a lower bound on the energy of all
     * conformations that contain a pruned rotamer, and; (b) prunedIsSteric[],
     * all conformations that are pruned due to unallowed sterics (boundKStar is
     * true).
     */
	...
	while (!done) {
		...
		for (int AA = 0; AA < numAAtypes[curPos]; AA++) {
			...
			// int index = curPos*numTotalRot + rotIndOffset[curAA]
			// + curRot;
			rotInMutInd.set(curPos, curAA, curRot, true); // rot
								      // index
								      // is in
								      // cur mut
								      // sequence
			...
			if ((boundKStar)
				|| (!eliminatedRotAtPos.get(curPos, curAA,
					curRot))) { // boundKStar or not already
						    // pruned

			    // logPS.println((curPos*numTotalRot +
			    // rotIndOffset[curAA] + curRot));logPS.flush();
			    curEc = RotamerSearch
			    ...
			    CanEliminate(curPos, curAA, curRot,
				    numRotForCurAAatPos);
			    indBounds.get(curPos, curAA, curRot).Ec = Math.min(
				    indBounds.get(curPos, curAA, curRot).Ec,
				    curEc); // update the lowest energy bound if
					    // necessary
			}
		    }
		}
	     ...
	     * 
	     * if ((boundKStar)||(!eliminatedRotAtPos[index])){ //boundKStar or
	     * not already pruned
	     * 
	     * curEc =
	     ...
	     * indBounds[numSiteResidues*numTotalRot+curRot].Ec =
	     * Math.min(indBounds[numSiteResidues*numTotalRot+curRot].Ec,
	     * curEc); //update the lowest energy bound if necessary } }
	     * System.out.println("done"); }
	     */

	    // KER: The quick sort was timing out so I replaced it with the
	    // Array sort
	    // TODO: Figure out why we sort this array at all, and why it is
		...
		// Determine the pruned rotamers: all rotamers whose lower
		// energy bound is above the given
		...
		while (iter.hasNext()) {
		    ...
		    RotInfo<MSRotBounds> ri = iter.next();
		    if (rotInMutInd.get(ri.state.index)) { // rot index is in
							   // current mutation
			numRot++;

			if (!boundKStar) {// pruning is performed

			    if ((!eliminatedRotAtPos.get(ri.state.index))) { // not
									     // already
									     // pruned

				if (ri.state.Ec > pruningE + Ew) { // higher
								   // than the
								   // cutoff
								   // energy, so
								   // prune
				    eliminatedRotAtPos
					    .set(ri.state.index, true);
				    prunedCurRun++;

				    if (ri.state.Ec >= stericE) { // pruned due
								  // to
								  // unallowed
								  // steric
								  // (from
			    ...
			    if (eliminatedRotAtPos.get(ri.state.index)) // check
									// the
									// Ec
									// among
									// all
			    ...
			    if (ri.state.Ec >= stericE) { // pruned due to
							  // unallowed steric
							  // (from all criteria
							  // used)
		...
		if (boundKStar)
		    Ec = Math.min(Ec, minEc); // the minimum pruned lower bound
					      // (among all pruned rotamers, for
					      // boundsKStar)

		if ((boundKStar) || (prunedCurRun == 0)) // boundKStar or no
							 // rotamers pruned this
							 // run, so done
		    done = true;
		else
		    numRuns++;
		...
		/*
		 * if (boundKStar){ for (int i=0; i<indBounds.length; i++){ if
		 * (rotInMutInd[indBounds[i].index]){
		 * System.out.print("rank: "+i
		 * +" rotIndex: "+indBounds[i].index+" minBound: "
		 * +indBounds[i].Ec);
		 * System.out.println(" pruned: "+eliminatedRotAtPos
		 * [indBounds[i].index]); } } }
		 */
	    }
	}
    ...
    // ComputeEliminatedRotConf() must be called first and onlyBound must be set
    // to true
    ...
    private void CanEliminate(int posNum, int AANumAtPos, int rotNumAtPos,
	    int numRotForCurAAatPos) {
	 ...
	 * only one position j here for which to add LigandIndMinMinPVE(atPos,
	 * withAA, withRot1); }
	 */
    }
	...
	if ((boundKStar)
		|| (!eliminatedRotAtPos.get(firstPos, firstAA, firstRot1))) { // boundsKStar
									      // or
									      // not
									      // pruned
		...
		for (int curRot = 0; curRot < numRotForAAatPos; curRot++) {
		    ...
		    if ((boundKStar)
			    || (!eliminatedRotAtPos.get(secondPos, curAA,
				    curRot))) { // boundsKStar or not pruned

			if ((boundKStar)
				|| ((!useFlags) || (!splitFlags[firstPos][firstAA][firstRot1][secondPos][curAA][curRot]))) { // boundKStar
															     // or
															     // (not
															     // using
															     // split
     ...
     * firstRot1;
     * 
     * if ((boundKStar)||(!eliminatedRotAtPos[index1])){ //boundsKStar or not
     * pruned
     * 
     ...
     * curLigPos;
     * 
     * if ((boundKStar)||(!eliminatedRotAtPos[index2])){ //boundsKStar or not
     * pruned
     * 
     * if ((boundKStar)||((!useFlags)||(!splitFlags[index1][index2]))){
     * //boundKStar or (not using split flags or not flagged)
     * 
     * curEmin =
	 ...
	 * one position j here for which to add if (withoutPos!=numSiteResidues)
	 * //if we are not currently checking ligand rotamers for pruning
	 * LigandMaxIndInt(); }
	 */
    }
	...
	if ((boundKStar) || (!eliminatedRotAtPos.get(atPos, atAA, atRot))) { // boundsKStar
									     // or
									     // not
									     // pruned
	 ...
	 * number of active site residues if (withoutPos!=numSiteResidues){ //if
	 * we are not currently checking ligand rotamers for pruning for (int
	 * curPos=0; curPos<numSiteResidues; curPos++){ if (curPos !=
	 * withoutPos){ LigandMaxPairInt(curPos); } } } }
	 */
    }
	...
	// int index1 = atPos1*numTotalRot + rotIndOffset[atAA1] + atRot1;//u at
	// k
	// int index2 = atPos2*numTotalRot + rotIndOffset[atAA2] + atRot2;//s at
	// j

	if ((boundKStar)
		|| ((!eliminatedRotAtPos.get(atPos1, atAA1, atRot1)) && (!eliminatedRotAtPos
			.get(atPos2, atAA2, atRot2)))) { // boundsKStar or not
							 // pruned

	    if ((boundKStar)
		    || ((!useFlags) || (!splitFlags[atPos1][atAA1][atRot1][atPos2][atAA2][atRot2]))) { // boundKStar
												       // or
												       // (not
												       // using
												       // split
     ...
     * 
     * if
     * ((boundKStar)||((!eliminatedRotAtPos[index1])&&(!eliminatedRotAtPos[index2
     * ]))){ //boundsKStar or not pruned
     * 
     * if ((boundKStar)||((!useFlags)||(!splitFlags[index1][index2]))){
     * //boundKStar or (not using split flags or not flagged)
     * 
     ...
     * 
     * double minIndVoxelE; double minShellResE;
     * 
     * minIndVoxelE =
     ...
     * return;
     * 
     * curEc += indInt[numSiteResidues]; //formula term 3 curEc +=
     * pairInt[numSiteResidues]; //formula term 5 SumMinDiffPVELig(curLigRot);
     * //formula term 4 }
     */
     ...
     * curPos=0; curPos<numSiteResidues; curPos++){ IndMinDiffPVELig(withRot1,
     * curPos); } }
     */
     ...
     * 
     * if ((boundKStar)||((!useFlags)||(!splitFlags[index1][index2]))){
     * //boundKStar or (not using split flags or not flagged) curEmin =
     * pairwiseMinEnergyMatrix
     * [numSiteResidues][ligAANum][firstRot1][secondPos][curAA][curRot]; minEmin
