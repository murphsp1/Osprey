...
public class VolModule {
    ...
    // in the search area
    Vector spacePoints; // Vector of surface space (double[3]) the x,y,z
			// coordinate
	...
	for (double q = bb[0]; q < bb[1]; q += spaceRes) {
		...
		for (double j = bb[4]; j < bb[5]; j += spaceRes) {
			...
			for (int yi = ypos - 1; yi <= ypos + 1; yi++) {
				...
				String key = new String(xi + "," + yi + ","
					+ zi);
				vec = (Vector) atomBuckets.get(key);
				if (!(vec == null))
				    for (int r = 0; r < vec.size(); r++)
					neighborAt[numAtomsToCheck++] = ((Integer) vec
		    ...
		    newspacept[1] = w;
		    newspacept[2] = j;
		    if (!tooClose(newspacept, neighborAt, numAtomsToCheck, -1,
			    0.95f)) {
			newspacept[3] = 0.0;
	...
	// the bounding region
	double possibleVolume = (bb[1] - bb[0]) * (bb[3] - bb[2])
		* (bb[5] - bb[4]);
	...
	for (int q = 0; q < numRes; q++) {
		...
		for (int w = 0; w < m.residue[q].numberOfAtoms; w++) {
		    ...
		    xpos = (int) Math.round(m.actualCoordinates[tmpNum]
			    / bucketResolution);
		    ypos = (int) Math.round(m.actualCoordinates[tmpNum + 1]
			    / bucketResolution);
		    zpos = (int) Math.round(m.actualCoordinates[tmpNum + 2]
			    / bucketResolution);
		    ...
		    vec.addElement(new Integer(
			    m.residue[q].atom[w].moleculeAtomNumber));
    ...
    // *************************************************************
    // Returns true if there is an atom too close to a specified new surface
    // point
    private boolean tooClose(double newsurfpt[], int neighborAt[],
	    int numAtomsToCheck, int moleculeAtomNumber, float threshMultiplier) {
	...
	for (int i = 0; i < numAtomsToCheck; i++) {
		...
		// Threshold is threshMultiplier * the distance of the atomic
		// radius
		// plus the solvent radius
		thresh = threshMultiplier
			* (m.atom[atNum].radius / 100.0 + solventRadius);
		thresh = thresh * thresh;
		if (((newsurfpt[0] - atCoords[0])
			* (newsurfpt[0] - atCoords[0])
			+ (newsurfpt[1] - atCoords[1])
			* (newsurfpt[1] - atCoords[1]) + (newsurfpt[2] - atCoords[2])
			* (newsurfpt[2] - atCoords[2])) < thresh)
		    return (true);
	    }
	}
	return (false);
