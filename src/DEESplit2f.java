...
public class DEESplit2f {
    ...
    // constructor
    DEESplit2f(float arpMatrix[][][][][][], float arpMatrixMax[][][][][][],
	...
	if (doMinimize && !doIMinDEE) // max matrix is different
	    ...
	    // no minimization, so the same matrix
	...
	if ((!eliminatedRotAtPos.get(posNum, AANumAtPos, rotNumAtPos))) { // not
		...
		boolean found = true;
		/*
		 * for (int i=0; i<majorSplitPos.length; i++){ if
		 * (i1==majorSplitPos[i]) found = false; }
		 */
		if ((i1 != posNum) && (found)) {
			...
			/*
			 * for (int i=0; i<majorSplitPos.length; i++){ if
			 * (i2==majorSplitPos[i]) found = false; }
			 */
			if ((i2 != posNum) && (found)) {
			    ...
			    int numPartitions = numRotForRes[splitPos[0]]
				    * numRotForRes[splitPos[1]];
			    boolean partitionPruned[] = new boolean[numPartitions]; // prune
										    // r
										    // at
										    // i
										    // for
			    ...
			    // sf1 is first indexed over the first residue; sf2
			    // is first indexed over the second residue;
			    ...
			    // get the mapping between rotamer number for the
			    // split residues and the index of that
			    // rotamer into the *boolean* matrix (1 is NOT
			    // added, in contrast to the PEM indices)
			    ...
			    // of the remaining rotamer possibilities to that of
			    // the given rotamer:
			    // given r at i, compare it to all t at i for
			    // pruning
				...
				if (!typeDependent || (altAA == AANumAtPos)) {
					...
					if (!((altAA == AANumAtPos) && (altRot == rotNumAtPos))) {
					    ...
					    // at this point, we know what r at
					    // i and t at i are
						...
						int partCount = 0; // the
								   ...
								   // partitions
								   // count
						int countFirst = 0; // the
								    // current
								    // rotamer
								    // for the
								    // first
								    // residue
						for (int spAA1 = 0; spAA1 < numAAtypes[splitPos[0]]; spAA1++) {// for
													       // each
													       // AA
													       // at
													       // the
						    ...
						    int partAA1 = strandRot[str0]
							    .getIndexOfNthAllowable(
								    strResNum0,
								    spAA1);
						    int numRotForPartAA1 = strandRot[str0].rl
							    .getNumRotForAAtype(partAA1);
						    if (numRotForPartAA1 == 0) // ala
									       // or
									       // gly
							numRotForPartAA1 = 1;

						    for (int partRot1 = 0; partRot1 < numRotForPartAA1; partRot1++) {// for
														     // each
														     // rot
														     // for
														     // the
							...
							// int index_hv1 =
							// splitPos[0]*numTotalRot
							// +
							// rotIndOffset[partAA1]
							// + partRot1; //the
							// index of the
							...
							if ((!eliminatedRotAtPos
								.get(splitPos[0],
									partAA1,
									partRot1))
								&& ((!useFlags) || (!splitFlags[posNum][AANumAtPos][rotNumAtPos][splitPos[0]][partAA1][partRot1]))) { // not
																				      // pruned

							    double splitPosDiffE1 = pairwiseMinEnergyMatrix[posNum][AANumAtPos][rotNumAtPos][splitPos[0]][partAA1][partRot1]
								    - pairwiseMaxEnergyMatrix[posNum][altAA][altRot][splitPos[0]][partAA1][partRot1]; // formula
																		      // term
																		      // 6

							    int countSecond = 0; // the
										 // current
										 // rotamer
										 // for
										 // the
										 // second
										 // residue
							    for (int spAA2 = 0; spAA2 < numAAtypes[splitPos[1]]; spAA2++) {// for
															   // each
															   // AA
															   // at
															   // the
								...
								int partAA2 = strandRot[str1]
									.getIndexOfNthAllowable(
										strResNum1,
										spAA2);
								int numRotForPartAA2 = strandRot[str1].rl
									.getNumRotForAAtype(partAA2);
								if (numRotForPartAA2 == 0) // ala
											   // or
											   // gly
								    numRotForPartAA2 = 1;

								for (int partRot2 = 0; partRot2 < numRotForPartAA2; partRot2++) {// for
																 // each
																 // rot
																 // for
																 // the
									...
									// index_hv2
									// =
									// splitPos[1]*numTotalRot
									// +
									// rotIndOffset[partAA2]
									// +
									// partRot2;
									// //the
									// index
									...
									if ((!eliminatedRotAtPos
										.get(splitPos[1],
											partAA2,
											partRot2))
										&& ((!useFlags) || (!splitFlags[posNum][AANumAtPos][rotNumAtPos][splitPos[1]][partAA2][partRot2]))) { // not
																						      // pruned

									    double splitPosDiffE2 = pairwiseMinEnergyMatrix[posNum][AANumAtPos][rotNumAtPos][splitPos[1]][partAA2][partRot2]
										    - pairwiseMaxEnergyMatrix[posNum][altAA][altRot][splitPos[1]][partAA2][partRot2]; // formula
																				      // term
																				      // 6
										...
										if (useFlags) {
										    sf1[countFirst - 1][countSecond - 1] = true;// incremented
																// above,
																// so
																// subtract
																// 1
						...
						if (useFlags) {
							...
							if (sfCount == sf1[0].length) { // r
											...
											// first
											// residue
							    splitFlags[index_r_AI.pos][index_r_AI.aa][index_r_AI.rot][indexMap1[partP1].pos][indexMap1[partP1].aa][indexMap1[partP1].rot] = true; // flag
																								  // as
																								  // a
																								  // dead-ending
																								  // pair
							...
							if (sfCount == sf2[0].length) { // r
											...
											// second
											// residue
							    splitFlags[index_r_AI.pos][index_r_AI.aa][index_r_AI.rot][indexMap2[partP1].pos][indexMap2[partP1].aa][indexMap2[partP1].rot] = true; // flag
																								  // as
																								  // a
																								  // dead-ending
																								  // pair
						...
						// after checking all
						// partitions, return if r at i
						// is pruned for all of the
						// partitions;
     ...
     * 
     * int str=mutRes2Strand[curRes]; int
     * strResNum=strandMut[str][mutRes2MutIndex[curRes]];
     * 
     * int indexMap[] = new int[numRotForRes[curRes]]; int indNum = 0; for (int
     * AA=0; AA<strandRot[str].getNumAllowable(strResNum); AA++){ //for each AA
     * for the given AS residue int curAA =
     * strandRot[str].getIndexOfNthAllowable(strResNum,AA); int numRotForAA =
     * strandRot[str].rl.getNumRotForAAtype(curAA); if (numRotForAA==0) //GLY or
     * ALA numRotForAA = 1;
     * 
     * for (int curRot=0; curRot<numRotForAA; curRot++){ //for each rot for the
     * given AA indexMap[indNum] = curRes*numTotalRot + rotIndOffset[curAA] +
     * curRot; indNum++; } } return indexMap; }
     */
    ...
    // KER: instead of returning the flat index return the curRes, curAA, and
    // curRot
	...
	int indNum = 0;
	for (int AA = 0; AA < strandRot[str].getNumAllowable(strResNum); AA++) { // for
										 // each
										 // AA
										 // for
										 // the
    ...
    private double SumMinDiffPVE(int atPos, int withAA1, int withRot1,
	    int withAA2, int withRot2, int splitPos1, int splitPos2) {
	...
	for (int curPos = 0; curPos < numMutable; curPos++) {
	    ...
	    if ((curPos != atPos) && (curPos != splitPos1)
		    && (curPos != splitPos2)) // j!=i and j!=k
