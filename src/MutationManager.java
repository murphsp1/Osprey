...
/**
 * The MutationManager class maintains a list of mutations to be tested,
 * maintains their scores, prints a log file, and generally manages the
 * mutations to test.
 */
public class MutationManager {
    ...
    String eRefMatrix;
    boolean PEMcomp = false; // true if PEM computation is performed; false if
			     // mut search is performed
	...
	distrDACS = false; // if this is a distributed DACS computation, the
			   // flag will be set with setDistrDACS()
	distrDEE = false; // if this is a distributed DEE computation, the flag
			  // will be set with setDistrDEE()
    }
	...
	if (PEMcomp) {// PEM computation
		...
		if (compASdist) { // AS-AS distance computation
		    ...
		    cObj.dist = dist;
			 ...
			 // TODO: fix this code so it makes sense for multiple
			 // strands
		    assert false == true;
		    cObj.flagMutType = mutArray[curMutIndex].flagMutType;
		    if (cObj.flagMutType.equalsIgnoreCase("AS-AS")) { // AS-AS
								      // run
			cObj.mutableSpots = 2;
		    ...
		    } else if (cObj.flagMutType.equalsIgnoreCase("INTRA")) { // INTRA
									     // run
			cObj.mutableSpots = 1;
	    ...
	    if ((!distrDEE) && (!distrDACS)) { // mutation search run, not
					       // (distributed DACS or
					       // distributed DEE)
	...
	if (PEMcomp) { // energy matrix computation
	    ...
	    // Update the E matrices computed so far with the new computations
	    // supplied
	    // by the current cObj
		...
		if (cObj.flagMutType.equals("INTRA")) {
			...
			eRef[cObj.compEE[i].i1][cObj.compEE[i].i2] = Math.min(
				eRef[cObj.compEE[i].i1][cObj.compEE[i].i2],
				cObj.compEE[i].minE);
		    }
		...
		if (compASdist) { // AS-AS distance computation
			...
			for (int i = 0; i < countNewEntries; i++) {
			    int index1 = 1
				    + rotLib.getRotamerIndexOffset()[cObj.compEE[i].i2]
				    + cObj.compEE[i].i3;
			    pairEMatrixMinEntropy[cObj.mutationNumber
				    * rotLib.getTotalNumRotamers() + index1][0] = cObj.compEE[i].minE;
			}
		    } else { // AS-AS run
			for (int i = 0; i < countNewEntries; i++) {
			    int ind1 = -1;
			    int ind2 = -1;
			    int index1 = cObj.compEE[i].i1
				    * rotLib.getTotalNumRotamers()
				    + rotLib.getRotamerIndexOffset()[cObj.compEE[i].i2]
				    + cObj.compEE[i].i3;
			    int index2 = cObj.compEE[i].i4
				    * rotLib.getTotalNumRotamers()
				    + rotLib.getRotamerIndexOffset()[cObj.compEE[i].i5]
				    + cObj.compEE[i].i6;
	    ...
	    /*
	     * System.out.println("MutNUM: "+cObj.curMut+" produced "+
	     * countNewEntries+" new entries.");
	     * logPS.print("Completed mutation "+cObj.curMut);
	     * logPS.print(" SlaveNum "+cObj.slaveNum);
	     * logPS.print(" Time "+(cObj.elapsedTime/60.0)); if (!entropyComp){
	     * for(int i=0;i<cObj.mutableSpots;i++)
	     * logPS.print(" "+cObj.resMut[i]);
	     * logPS.print(" "+cObj.flagMutType); } logPS.println();
	     * logPS.flush();
	     */
	} else { // mutation search
	    if ((!distrDEE) && (!distrDACS)) { // Hybrid MinDEE-K*, not
					       // (distributed DACS or
					       // distributed DEE)
		...
		for (int i = 0; i < cObj.numComplexes; i++) {
		    logPS.print(" " + i + "ConfInfo "
			    + cObj.searchNumConfsEvaluated[i] + " "
			    + cObj.searchNumPrunedMinDEE[i] + " "
			    + cObj.searchNumConfsPrunedByS[i] + " "
			    + cObj.searchNumConfsLeft[i]);
		}
		...
		if (score.compareTo(bestScore) > 0) {
		    logPS.println("BestScoreChange " + bestScore + " to "
			    + score);
		    bestScore = score;
		}
		logPS.flush();
		...
		logPS.println();
		logPS.flush();
		System.out.println("Partition " + cObj.mutationNumber
			+ " done; best energy: " + cObj.bestScore);
	    } else {// distributed DEE

		if (typeDEE == optPairs) { // pairs DEE
		    boolean tmpSF[][][][][][] = null;
		    tmpSF = (boolean[][][][][][]) readFromFile(tmpSF,
			    cObj.sfFileOut);
		    ...
		    /*
		     * for (int i1=0; i1<tmpSF.length; i1++){ for (int i2=0;
		     * i2<tmpSF[0].length; i2++){ if (tmpSF[i1][i2]) //get the
		     * DE pairs from this computation splitFlags[i1][i2] = true;
		     * } }
		     */
		    deleteFile(cObj.sfFileOut);
		    outputObject(splitFlags, sfFile); // must be output after
						      // each result read
		} else { // singles DEE
			...
			if (cObj.prunedRot.get(ri)) // get the pruned rotamers
						    // from this computation
			    prunedRot.set(ri, true);
		    }
		    outputObject(prunedRot, rotFile); // must be output after
						      // each result read
		}
	    }
	}
	...
	try {
	    ...
	    System.out
		    .println("ERROR: An exception occurred while reading from object file");
	    System.exit(0);
	}
	return inObj;
    }
	...
	try {
		...
		System.out.println(e.toString());
		System.out.println("ERROR: unable to release lock on file "
			+ outFile);
		System.exit(1);
	    }
	}
    ...
    /*
     * public void setLigType(String lt) { ligType = lt; }
     */
    ...
    /*
     * public void setNumInAS(int nas) { numInAS = nas; }
     */
    ...
    /*
     * public void setnumLigRotamers(int nlr) { numLigRotamers = nlr; }
     */
