...
public class MSAStar {
	...
	private void printState(QueueNode expNode) {
		...
		return;
		/*
		 * numExpanded++; if (expNode.level + 1 > topL) { topL = expNode.level +
		 * 1; numTopL = 1; } else if (expNode.level + 1 == topL) numTopL++;
		 * 
		 * if ((numExpanded % 1000) == 0) {
		 * System.out.print(curExpansion.numNodes + " " + expNode.fScore + " " +
		 * expNode.level + " "); for (int i = 0; i < expNode.level; i++)
		 * System.out.print(expNode.confSoFar[i] + " "); System.out.println(topL
		 * + " " + numTopL); }
		 */
	}
