...
/**
 * Implements functions for checking the steric overlap for specified parts of a
 * given structure at different stages of the A* expansion of the conformation
 * tree.
 * 
 */
public class StericCheck {
    ...
    // the number of conformations above each level (from level i+1 to the top
    // level,
    // which is the ligand)
    ...
    /*
     * StericCheck (int curAANumP[],int curResToASMapP[],int
     * strandMutP[],boolean eliminatedRotAtPosRedP[], int
     * numRotForResP[],Molecule mP, double overlapThreshP, boolean hS,
     * BigInteger numConfsLeftP, BigInteger numConfsAboveLevelP[], int
     * numMutableP, StrandRotamers sysLRP, int ligStrNumP, StrandRotamers
     * ligROTP, int curLigAANum, RotamerLibrary rlP, RotamerLibrary grlP){
     * 
     * curAANum = curAANumP; //curResToASMap = curResToASMapP; //residueMap =
     * residueMapP; //numInAS = residueMap.length; strandMut = strandMutP; m =
     * mP; overlapThresh = overlapThreshP; hSteric = hS; eliminatedRot =
     * eliminatedRotAtPosRedP; numRotForRes = numRotForResP; numConfsLeft =
     * numConfsLeftP; numConfsAboveLevel = numConfsAboveLevelP;
     * numConfsPrunedByS = new BigInteger("0"); sysStrNum = sysStrNumP; sysLR =
     * sysLRP; ligStrNum = ligStrNumP; ligROT = ligROTP; ligAANum = curLigAANum;
     * ligPresent = true; rl = rlP; grl = grlP;
     * 
     * //logPS = logPSP; }
     */

    StericCheck(int curAANumP[],/* int curResToASMapP[], */int strandMutP[][],
	    boolean eliminatedRotAtPosRedP[], int numRotForResP[], Molecule mP,
	    double overlapThreshP, boolean hS, BigInteger numConfsLeftP,
	    BigInteger numConfsAboveLevelP[], int numMutableP,
	    int numberOfStrandsP, StrandRotamers strandRotP[],
	    int mutRes2StrandP[], int mutRes2StrandMutIndexP[]) {
    ...
    // Sets up the steric check for the given partial (full) conformation:
    // curConf[] has been
    // assigned for levels 0...(curTopLevel-1) and curNode should be applied at
    // curTopLevel;
    // Returns true if the partial conformation is sterically allowed
    // Called by A*
    public boolean checkAllowedSteric(int curTopLevel, int curConf[],
	    int curNode) {

	// As the rotamers given to A* are only the non-pruned ones, there is a
	// difference between the
	// rotamer numbers returned by A* and the actual rotamer numbers for
	// each residue (that is,
	// A* may return rot 4 for res 3, but rot 3 for res 3 may be pruned, and
	// so the actual number
	// of the rot to be applied for res 3 is 5)
	int curPruningInd = 0;
	int curRotInd, compInd;
	int conf[] = new int[curTopLevel + 1];
	for (int curRes = 0; curRes <= curTopLevel; curRes++) {// compute the
							       // actual rot
							       // numbers for
							       // levels
							       // 0...curTopLevel
		...
		if (!eliminatedRot[curPruningInd]) {
		    if (curRes == curTopLevel) // the rotamer (curNode) for
					       // curTopLevel is not a part of
					       // curConf[]
			compInd = curNode;
		    else
	...
	// Backup the atom coordinates, so that they can be restored after the
	// steric check, as
	// applyRotamer() changes both the actualCoordinates[] and the atom
	// coordinates,
	// so we cannot restore the original position just using
	// m.updateCoordinates()
	m.backupAtomCoord();

	// Apply the rotamers of the current partial conformation (up to level
	// (curTopLevel-1))
	int curAS = 0;
	boolean applyLig = false;

	// If there is a ligand and curTopLevel is the ligand level (full
	// conformation), apply the lig rotamer;
	...
	/*
	 * for (int curRes=0; curRes<m.strand[sysStrNum].numberOfResidues;
	 * curRes++){ if (curAS<curTopLevel){ //apply for AS res
	 * 0...(curTopLevel-1) if (curResToASMap[curRes]!=-1){//make a change
	 * only to the AS residues: use the native type for the other residues
	 * 
	 * if (rl.getNumRotForAAtype(curAANum[residueMap[curAS]])!=0){//not GLY
	 * or ALA sysLR.applyRotamer(m, curRes, conf[curAS]); } curAS++;
	 * //prepare the next AS residue } } else if (!applyLig) { //we need to
	 * apply an AS rot at curTopLevel, as there is no ligand if
	 * (curResToASMap[curRes]!=-1){ if
	 * (rl.getNumRotForAAtype(curAANum[residueMap[curAS]])!=0)//not GLY or
	 * ALA sysLR.applyRotamer(m, curRes, conf[curTopLevel]); break; } } else
	 * //we have already applied all of the rotamers for the given partial
	 * conformation break; }
	 */
	 ...
	 * ); for (int
	 * i=0;i<=curTopLevel;i++)logPS.print(conf[i]+" ");logPS.println(); if
	 * (!ligPresent){ for (int
	 * i=0;i<=curTopLevel;i++)logPS.print(sysLR.getCurRotNum
	 * (residueMap[i])+" ");logPS.println();logPS.flush(); } else{ for (int
	 * i
	 * =0;i<curTopLevel;i++)logPS.print(sysLR.getCurRotNum(residueMap[i])+" "
	...
	/*
	 * if ((ligPresent)&&(curTopLevel==numInAS)) //check the ligand (which
	 * is at the top level) against all other residues allowedSteric =
	 * RS_CheckAllSterics(ligStrNum,0); else
	 */
	...
	if (!allowedSteric) { // decrease the number of remaining conformations
	    numConfsLeft = numConfsLeft
		    .subtract(numConfsAboveLevel[curTopLevel]);
	    numConfsPrunedByS = numConfsPrunedByS
		    .add(numConfsAboveLevel[curTopLevel]);
	}
	...
	for (int i = 0; i < res.numberOfAtoms; i++) {
		...
		for (int q = 0; q < m.numberOfStrands; q++) {
			...
			if (!((q == strandNum) && (w == resNum))) {// not the
								   // same
								   // residue
			    if (!(((q > strandNum) && curASToResMap[q][w]) || ((q == strandNum)
				    && (w > resNum) && (curASToResMap[q][w])))) {
				// if
				// ((!((strandNum==sysStrNum)&&(q==strandNum)&&(w>resNum)&&(curASToResMap[w])))&&
				// (!((q==ligStrNum)&&(strandNum==sysStrNum)))){//not
				// an AS residue with a bigger res number AND
				// not the ligand
			    ...
			    } else if (((q > strandNum) && curASToResMap[q][w])
				    || ((q == strandNum) && (w > resNum) && (curASToResMap[q][w]))) {
				// else if
				// ((strandNum==sysStrNum)&&(q==strandNum)&&(w>resNum)&&(curASToResMap[w])){
				// //an unassigned AS residue, so check only the
