...
/**
 * Handles the application of the Richardsons' Backrub motions for a given
 * residue in a protein
 * 
 */
public class Backrubs implements Serializable {

    final float thetaSmallScale = 0.7f; // the scaling factor for the rotation
					// angles for the small rotations
    ...
    // Applies a Backrub motion to residue resNum (strand-relative numbering) of
    // strand strandNum in molecule m;
    // This motion includes three rotations:
    // 1) rotate resNum and its adjacent peptides by theta1 degrees around the
    // CA(resNum-1)-CA(resNum+1) axis;
    // 2) rotate the peptide with resNum-1 by theta2 degrees around the
    // CA(resNum-1)-CA(resNum) axis;
    // 3) rotate the peptide with resNum+1 by theta3 degrees around the
    // CA(resNum)-CA(resNum+1) axis;
    // The only atoms that move from their initial positions belong to residue
    // resNum and the adjacent peptides
    // If computeSmallRot is true, then the values of theta2 and theta3 are
    // computed after the first rotation,
    // so that the H-bonding positions of CO and NH are roughly preserved
    public float[] applyBackrub(Molecule m, int strandNum, int resNum,
	    float theta1, boolean computeSmallRot, float theta2, float theta3) {

	if (((resNum == 0) || (m.strand[strandNum].residue[resNum - 1]
		.getResNumber() != m.strand[strandNum].residue[resNum]
		.getResNumber() - 1))
		|| ((resNum == (m.strand[strandNum].numberOfResidues - 1)) || (m.strand[strandNum].residue[resNum + 1]
			.getResNumber() != m.strand[strandNum].residue[resNum]
			.getResNumber() + 1))) {
	    System.out.println("ERROR: Backrubs cannot be applied for residue "
		    + m.strand[strandNum].residue[resNum].getResNumber());
	    System.exit(1);
	}
	...
	// Get the three lists of atoms to rotate (the atom lists are in
	// molecule-relative numbering):
	// 1) C, O (prevRes); all atoms (curRes); N, H (nextRes);
	// 2) C, O (prevRes); N, H (curRes);
	// 3) C, O (curRes); N, H (nextRes)
	int atomList1[] = new int[curRes.numberOfAtoms + 4]; // list of atoms to
							     // be rotated
	...
	Atom pseudoOprevOld = new Atom(Oprev.name, OprevOldCoord[0],
		OprevOldCoord[1], OprevOldCoord[2]);
	float OcurOldCoord[] = getActualCoord(m, Ocur);
	Atom pseudoOcurOld = new Atom(Ocur.name, OcurOldCoord[0],
		OcurOldCoord[1], OcurOldCoord[2]);

	// Perform the big rotation
	rotateList(m, atomList1, CAprev, CAnext, CAprev, theta1);

	// Determine the rotation angles for the two smaller rotations, in order
	// to roughly preserve the H-bonding positions of CO and NH
	if (computeSmallRot) {
    ...
    // Rotates all atoms in atomList[] around the axis between atoms a1-a2,
    // centered at atom c, by theta degrees
    private void rotateList(Molecule m, int atomList[], Atom a1, Atom a2,
	    Atom c, float theta) {
	...
	float dx = m.actualCoordinates[a2num * 3]
		- m.actualCoordinates[a1num * 3];
	float dy = m.actualCoordinates[a2num * 3 + 1]
		- m.actualCoordinates[a1num * 3 + 1];
	float dz = m.actualCoordinates[a2num * 3 + 2]
		- m.actualCoordinates[a1num * 3 + 2];
	...
	m.rotateAtomList(atomList, dx, dy, dz, center[0], center[1], center[2],
		theta, false);
    }

    // Find the atName atom of the given residue and store in at[ind] and
    // atNum[ind]
    ...
    // Get the small rotation angle that will rotate atom a1 around the axis
    // defined by atoms (a2,a3), so that a1 will be as close as possible to atom
    // a4
    private float getSmallRotAngle(Molecule m, Atom a1, Atom a2, Atom a3,
	    Atom a4) {
    ...
    // Returns the projection (a pseudo-atom) of atom p3 onto the line between
    // atoms l1 and l2
    private Atom projectPointLine(Atom l1, Atom l2, Atom p3) {

	float c[] = new float[3];
	double d12sq = Math.pow(l2.distance(l1), 2);
	double u = ((p3.coord[0] - l1.coord[0]) * (l2.coord[0] - l1.coord[0])
		+ (p3.coord[1] - l1.coord[1]) * (l2.coord[1] - l1.coord[1]) + (p3.coord[2] - l1.coord[2])
		* (l2.coord[2] - l1.coord[2]));
	u = u / d12sq;
    ...
    // Returns the projection (a pseudo-atom) of atom p4 onto the plane with
    // normal defined by atoms (l1,l2) and a point on that plane c3
    ...
    // Compute the closest point on the circle defined by the Atom c (center)
    // and radius (c,p1) to Atom q1 (this assumes c, p1, and q1 are coplanar)
