...
public class StrandRotamers implements Serializable {
    ...
    private int allowableAAs[][]; // Which AA types are allowable at each
				  // position
    private int numAllowable[]; // Number of allowables
    private String curAAType[] = null; // the three-letter code currently
				       // assumed by each residue in the strand
    private int curRotNum[] = null; // the rotamer number currently assumed by
				    // each residue in the strand
    ...
    // Returns the current three-letter code for the specified residue index
    // strResPos (strand-relative numbering)
    public String getCurRotType(int strResPos) {
	return (curAAType[strResPos]);
    }

    // Returns the current rotamer number for the specified residue index
    // strResPos (strand-relative numbering)
    ...
    public void getAtomsMoreDistal(Molecule m, int resNum, Atom at2,
	    int atomList[]) {
    ...
    // This function is called when CANew is already aligned with CAOld and NNew
    // is
	...
	if (debug)
		...
		System.out
			.println("Warning: LovellRotamers: fp is small but rotation is not 180!");
    ...
    // This function converts residue (resNum) to the conformation specified by
    // rotamer rotNum
    public boolean applyRotamer(Molecule m, int resNum, int rotNum) {

	int aaNum = rl
		.getAARotamerIndex(m.strand[strandNumber].residue[resNum].name);
	...
	for (int i = 0; i < aaDihedrals; i++) {
		...
		for (int w = 0; w < 4; w++) {
		    if (localResidue.atom[q].name.equalsIgnoreCase(rl
			    .getDihedralAtomNames(aaNum, i, w))) {
	    ...
	    getAtomsMoreDistal(m, localResidue.moleculeResidueNumber, at[2],
		    atomList);
    ...
    // residue
    public void changeResidueType(Molecule m, int resNum, String newResType,
    ...
    public void changeResidueType(Molecule m, int resNum, String newResType,
	    boolean addHydrogens, boolean connectResidue) {

	// call with useOldBBatoms = false
	changeResidueType(m, resNum, newResType, addHydrogens, connectResidue,
		false);
    }
    ...
    // If useOldBBatoms is true, then the *exact* coordinates for the old
    // backbone
    // CA, C, and CB atoms are copied for the new residue
    //
    // This function makes use of the CB carbon, unfortunately if we are
    // changing
    ...
    //
    // In step 3), we could align the backbone C's, but due to differences
    // between
    ...
    public void changeResidueType(Molecule m, int resNum, String newResType,
	    boolean addHydrogens, boolean connectResidue, boolean useOldBBatoms) {
	...
	/*
	 * KER: Histidine can now take on the forms HIP, HIE, and HID so it
	 * should be allowed to change between the different types if
	 * (isResHis(localResidue.name)&&isResHis(newResType)) // checks if the
	 * new and old types are both forms of histidine, if so a mutation is
	 * not done return; // This prevents say HID from reverting into HIS
	 */

	// If the old or new residues are glycine then we must do special things
	// as we treat the H as CB
	...
	if (oldResGly) { // we didn't find CBOld as Gly doesn't have it; find
			 // HA3 and point CBOld to it
	    for (int q = 0; q < localResidue.numberOfAtoms; q++) {
		if ((localResidue.atom[q].name.equalsIgnoreCase("HA3"))
			|| (localResidue.atom[q].name.equalsIgnoreCase("3HA")))
		    CBOld = localResidue.atom[q];
	    }
	    if (CBOld == null) {
		System.out.println("HA3 on " + localResidue.fullName
			+ " couldn't be found. Please check labelling.");
		System.out.println("Going to assume we want HA2");
		for (int q = 0; q < localResidue.numberOfAtoms; q++) {
		    if ((localResidue.atom[q].name.equalsIgnoreCase("HA2"))
			    || (localResidue.atom[q].name
				    .equalsIgnoreCase("2HA")))
			CBOld = localResidue.atom[q];
		}
	    }
	}
	...
	if (CAOld.distance(CANew) > 0.0001) {
	    ...
	    for (int q = 0; q < r.numberOfAtoms; q++)
		atomList[q] = q;
	    r.rotateResidue(NNew, rotAxis[0], rotAxis[1], rotAxis[2],
		    -thetaDeg[0], atomList, numAtoms);
	}

	// Rotate CBs to overlap - now the residue backbones should be aligned
	if ((!glyMutation) && (CBOld.distance(CBNew) > 0.0001)) { // not a to-
								  // or from-
								  // Gly
								  // mutation
	    ...
	    for (int q = 0; q < r.numberOfAtoms; q++)
		atomList[q] = q;
	    r.rotateResidue(CANew, rotAxis[0], rotAxis[1], rotAxis[2],
		    -thetaDeg[0], atomList, numAtoms);
	}
	...
	// Make the positions of the new backbone atoms coincide *exactly* with
	// the old ones;
	// NNew already has the same coordinates as NOld;
	// the CAs, CBs, and CBs are already aligned, but may differ due to
	// differences between the template PPR and the residue backbone in the
	// PDB
	if (useOldBBatoms) {
	    setAtomCoord(CANew, CAOld);
	    if ((CBNew != null) && (CBOld != null)) // not a from/to Gly
						    // mutation (Gly does not
						    // have a CB)
	...
	if (localResidue.nterm) { // we are changing the nterm residue
	    // we must add the H1, H2, H3 atoms (and delete the HN atom) to the
	    // new residue,
	...
	if (localResidue.cterm) { // we are changing the cterm residue
	    // we must add the OXT or H1, H2, H3 atoms (and delete the HN atom)
	    // to the new residue,
	    // since the PPR templates only handle polypeptide residues
	...
	if ((resNum + 1) < m.strand[strandNumber].numberOfResidues) {
	    if (connectResidue)
		connectNextResidue = (localResidue.getResNumber() + 1 == m.strand[strandNumber].residue[resNum + 1]
			.getResNumber());
	}
	if (resNum > 0) {
	    if (connectResidue)
		connectLastResidue = (localResidue.getResNumber() - 1 == m.strand[strandNumber].residue[resNum - 1]
			.getResNumber());
	    if (connectLastResidue) {
		for (int q = 0; q < m.strand[strandNumber].residue[resNum - 1].numberOfAtoms; q++) {
		    if (m.strand[strandNumber].residue[resNum - 1].atom[q].name
			    .equalsIgnoreCase("C"))
	...
	if (r.name.length() > 3)
	    localResidue.fullName = new String(r.name.substring(0, 3) + " "
		    + localResidue.fullName.substring(4));
	...
	for (int j = 0; j < m.numberOfStrands; j++) {
		...
		for (int w = 0; w < m.strand[j].residue[q].numberOfAtoms; w++) {
			...
			if (m.strand[j].residue[q].atom[w].name
				.equalsIgnoreCase("C")
				&& (nextResidueBase != -1)
				&& connectNextResidue) {
			    // mark the atoms to bond so we can join when we're
			    // done
			...
			if (m.strand[j].residue[q].atom[w].name
			    ...
			    m.addBondBetween(
				    lastResidueC,
				    m.strand[j].residue[q].atom[w].moleculeAtomNumber);
			}
		    } else {
			if ((tmpIntAry != null) && (nextResidueBase != -1)) {
			    for (int i = 0; i < m.strand[j].residue[q].atom[w].bond.length; i++) {
				// Because the position of this residue's
				// backbone C atom can move
				// we have to be careful about simply updating
				// the bond numbers.
				// The backbone N of numRes+1 if it bonds back
				// to this residue needs
				// special attention
				// Basically if there is a bond to the old
				// residue we'll delete that
				// bond. The only types of bonds into the new
				// residue should be
				// the previous and next peptide bond which we
				// handle ourselves.
				if (tmpIntAry[i] >= nextResidueBase)
				    tmpIntAry[i] += changeInAtoms;
				else if (tmpIntAry[i] >= thisResidueBase) {
				    m.strand[j].residue[q].atom[w]
					    .deleteBond(i);
				    tmpIntAry = m.strand[j].residue[q].atom[w].bond;
				    i--;
				}
     ...
     * 
     * if (res.equalsIgnoreCase("HID") || res.equalsIgnoreCase("HIE") ||
     * res.equalsIgnoreCase("HIP") || res.equalsIgnoreCase("HIS")) return true;
     * else return false; }
     */

    // Returns a handle on the backbone N, CA, C, O, H, and CB atoms for residue
    // res
	...
	for (int q = 0; q < res.numberOfAtoms; q++) {
		...
		at[2] = res.atom[q];
		    ...
		    || res.atom[q].name.equalsIgnoreCase("O1"))
		at[3] = res.atom[q];
    ...
    // Sets the coordinates of atom toAt to be the same as the coordinates of
    // atom fromAt
    ...
    // This function returns a modified version of residue r by replacing the
    // single backbone H atom
    // with the H1, H2, and H3 atoms from localResidue, and updaing the bond
    // information
	...
	for (int q = 0; q < localResidue.numberOfAtoms; q++) {
		...
		Atom at1 = new Atom(lAtom.name, lAtom.coord[0], lAtom.coord[1],
			lAtom.coord[2], lAtom.charge, lAtom.forceFieldType);
		r1.addAtom(at1);
	    }
	}
	...
	for (int q = 0; q < r1.numberOfAtoms; q++) {
	    if ((r1.atom[q].name.equalsIgnoreCase("H1"))
		    || (r.atom[q].name.equalsIgnoreCase("H2"))
		    || (r1.atom[q].name.equalsIgnoreCase("H3"))) {

		r1.atom[q].addBond(0); // add the bond for this H atom to the N
				       // atom
		r1.atom[0].addBond(q); // add the bond for the N atom to this H
				       // atom
	    }
	}
	...
	for (int q = 0; q < r1.numberOfAtoms; q++) {
	    if (r1.atom[q].name.equalsIgnoreCase("OXT")
		    || r1.atom[q].name.equalsIgnoreCase("O2")) {
    ...
    // Performs the CB alignment (changes the coordinates of the new residue r)
    // for the new residue when the old residue is Gly
    private void alignCBOldGly(Atom CBOld, Atom CBNew, Atom CAOld, Atom CANew,
	    Atom NOld, Residue r) {
	...
	try {
	    ...
	    // Compute the angle between CBOld-CA and CBNew-CA; here, CBOld is
	    // actually HA3
	    ...
	    double angle = 180.0 / 3.1415 * Math
		    .acos(dotProd / magOld / magNew);
    ...
    // Performs the CB alignment (changes the coordinates of the new residue r)
    // when the new residue is Gly
    private void alignCBNewGly(Atom CBOld, Atom CBNew, Atom CAOld, Atom CANew,
	    Atom NOld, Residue r, Residue localResidue) {
	...
	for (int q = 0; q < r.numberOfAtoms; q++) {
	    if (r.atom[q].name.equalsIgnoreCase("HA2")
		    || r.atom[q].name.equalsIgnoreCase("2HA")) {
	...
	localH = -1;
	for (int q = 0; q < r.numberOfAtoms; q++) {
	    if (r.atom[q].name.equalsIgnoreCase("HA3")
		    || r.atom[q].name.equalsIgnoreCase("3HA"))
		localH = q;
	}
	Atom tempH = r.atom[localH];
	double magNew = Math
		.sqrt(((tempH.coord[0] - CANew.coord[0]) * (tempH.coord[0] - CANew.coord[0]))
			+ ((tempH.coord[1] - CANew.coord[1]) * (tempH.coord[1] - CANew.coord[1]))
			+ ((tempH.coord[2] - CANew.coord[2]) * (tempH.coord[2] - CANew.coord[2])));
	double magOld = Math
		.sqrt(((CBOld.coord[0] - CANew.coord[0]) * (CBOld.coord[0] - CANew.coord[0]))
			+ ((CBOld.coord[1] - CANew.coord[1]) * (CBOld.coord[1] - CANew.coord[1]))
			+ ((CBOld.coord[2] - CANew.coord[2]) * (CBOld.coord[2] - CANew.coord[2])));
	r.atom[localH].coord[0] = (float) (CANew.coord[0] + (CBOld.coord[0] - CANew.coord[0])
		* magNew / magOld);
	r.atom[localH].coord[1] = (float) (CANew.coord[1] + (CBOld.coord[1] - CANew.coord[1])
		* magNew / magOld);
	r.atom[localH].coord[2] = (float) (CANew.coord[2] + (CBOld.coord[2] - CANew.coord[2])
		* magNew / magOld);
    }
