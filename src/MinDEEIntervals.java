...
public class MinDEEIntervals {
    ...
    // the distance between the closest side-chain atoms of each pair of active
    // site residues (the ligand is not considered here)
    ...
    // constructor
    MinDEEIntervals(float arpMatrix[][][][][][],
	    float arpMatrixMax[][][][][][], int numResMutable, int strMut[][],
	    StrandRotamers strandLRot[],
	    PrunedRotamers<Boolean> prunedRotAtRes, boolean scInt,
	    float dist[][], float maxSc, int mutRes2StrandP[],
	...
	if (!scaleInt)
	    ...
	    /*
	     * if ((pos1>=numSiteResidues)||(pos2>=numSiteResidues)) //one of
	     * the residues is the ligand, so scale with maxScale return
	     * maxScale; else { //both residues are active site residues
	     */
		...
		float sf = maxScale
			* (float) Math
				.pow((1 - (float) Math.pow(
					(asResDist[pos1][pos2] / maxDist),
					(1 / k))), k);
		return sf;
	    }
	    // }
	 ...
	 * (withoutPos!=numSiteResidues) //if we are not currently checking
	 * ligand rotamers for pruning sum += maxScale*LigandMaxIndInt(); }
	 */
	...
	if (!eliminatedRotAtPos.get(atPos, atAA, atRot)) {
	    ...
	    // the interval is always non-negative, so returning 0.0 if pruned
	    // is correct
	    return 0.0;
    }
     ...
     * 
     * return ((maxE+maxShell) - (minE+minShell)); } else //the interval is
     * always non-negative, so returning 0.0 if pruned is correct return 0.0; }
     */
    // ////////////////////////////////////////////////////////////////////////////////
	...
	if ((!eliminatedRotAtPos.get(atPos1, atAA1, atRot1))
		&& (!eliminatedRotAtPos.get(atPos2, atAA2, atRot2))) {
     ...
     * 
     * return (maxE - minE); } else //the interval is always non-negative, so
     * returning 0.0 if pruned is correct return 0.0; }
     */
}
