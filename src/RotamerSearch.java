...
//// BEGIN HELPER FUNCTION SECTION
	...
	public float computeBestRotEnergyBound(/*int numTotalRotamers, int rotamerIndexOffset[]*/) {
		...
		for(int i=0;i<ASAANums.length;i++) {
			bestE += arpMatrix[i][ASAANums[i]][curStrRotNum[i]][i][0][0]; //Add the intra-rotamer energy
			bestE += RotamerSearch.getShellRotE(arpMatrix, i, ASAANums[i], curStrRotNum[i]);//arpMatrix[i][ASAANums[i]][curStrRotNum[i]][i][0][1]; // Add the rotamer-shell energy and Add the intra-rotamer energy
...
//// BEGIN PAIRWISE MUTATION _ALL_ ROTAMER SEARCH SECTION - ENERGY PRECOMPUTATION
	...
	private void computeIntraRotEnergies(int strandMut[][],	float retEMatrixMin[][][][][][],
			float retEMatrixMax[][][][][][], int residueMutatable[], 
		...
		for(int i=0; i<residueMutatable.length;i++){
			...
			if (residueMutatable[i] == 1){ //only compute for those residues that are mutable
				...
				for(int j=0;j<strandRot[str].getNumAllowable(strResNum);j++){
					...
					boolean done = false;
					int curRot = 0;
					int totRotForCur = strandRot[str].rl.getNumRotForAAtype(curAANum[molResNum]);
		...
		if (curMut>=numMut){// If we've arrived here, then we have assigned a full amino acid sequence and we are ready to generate all rotamer combinations
			...
			/*if (ligPresent){
				...
				pairwiseMutationAllRotamerSearch(maxDepth, strandMut, res1, res2, res1Num,
					res2Num,-1,-1,retEMatrixMin, retEMatrixMax, minimizeBB, doBackrubs, numMut, mutDepth, 0);
			//}
	...
	private void pairwiseMutationAllRotamerSearch(int maxDepth, int strandMut[][],
		...
		//else {
			if (curMut>=numMut) {
				...
				int numRot = strandRot[str].rl.getNumRotForAAtype(curAANum[molResNum]);
	...
	private void pairwiseRotamerEnergyBackrubsHelper (int res1,int res2,int res1Strand,int res1Resnum,int res2Strand,int res2Resnum,
			int strandMut[][], boolean shellRun,float retEMatrixMin[][][][][][], float retEMatrixMax[][][][][][],boolean templateOnly,
			int res1AANum, int res1RotNum, int res2AANum, int res2RotNum){
...
//// BEGIN ROTAMER SEARCH SECTION
	...
	private int countConfs(int numMutable, int numRes, /*boolean ligPresent,*/ 
		...
		for (int curLevel=0; curLevel<numRes; curLevel++){
			...
			curNumRot = strandRot[str].rl.getNumRotForAAtype(curAANum[molResNum]);
	...
	private void slaveRotamerSearchAStar(int numMutable, int strandMut[][], boolean minimizeBB,
		...
		arpMatrixRed = new float[numTotalRotRedNonPruned+arpMatrix[arpMatrix.length-1][0][0][0][0].length][numTotalRotRedNonPruned+1];//include the intra-energies in the last column
		...
		reduceMatrices(eliminatedRotAtPosRed, arpMatrixRed,	indicesEMatrixPos, indicesEMatrixAA, indicesEMatrixRot, numRotForRes, numRotForResNonPruned, 
				treeLevels, numTotalRotRedNonPruned, numMutable, strandMut);
		...
		if (!minimizeBB) {//do not do a steric check if backbone minimization
			/*if (ligPresent)
				...
				stericF = new StericCheck(curAANum,/*curResToASMap,*/strandMut,eliminatedRotAtPosRed,numRotForRes,
						m,softOverlapThresh,hSteric,numConfsLeft,numConfsAboveLevel,numMutable,numberOfStrands,strandRot,mutRes2Strand,mutRes2StrandMutIndex);
		}
	...
	private void reduceMatrices(boolean eliminatedRotAtPosRed[], float arpMatrixRed[][], 
		...
		for (int curLevel=0; curLevel<treeLevels; curLevel++){
			...
			for (int curRot=0; curRot<numRotForRes[curLevel]; curRot++){
				/*if ((ligPresent)&&(curLevel==(treeLevels-1))){ //the ligand level
					...
					//int curIndex = curLevel*numTotalRotamers + rotamerIndexOffset[curAANum[molResNum]] + curRot;
				if (!eliminatedRotAtRes.get(curLevel, curAANum[molResNum], curRot)){ //not pruned, so add its index
						indicesEMatrixPos[curIndexRed] = curLevel;
						indicesEMatrixAA[curIndexRed] = curAANum[molResNum];
						indicesEMatrixRot[curIndexRed] = curRot;
						curIndexRed++;
					}
					eliminatedRotAtPosRed[pruningIndex] = eliminatedRotAtRes.get(curLevel, curAANum[molResNum], curRot);
					//logPS.println(pruningIndex+" "+curIndex+" "+eliminatedRotAtRes[curIndex]);logPS.flush();
					pruningIndex++;
				//}				
			}
		...
		for (int curRot1=0; curRot1<numTotalRotRedNonPruned; curRot1++){
			...
			for(int i=1; i<arpMatrix[p1][a1][r1][p1][0].length;i++)
				arpMatrixRed[numTotalRotRedNonPruned+(i-1)][curRot1] = arpMatrix[p1][a1][r1][p1][0][i];//store shell-rotamer E in the last rows
		}
	}
 ...
 */		
	...
	public void doCompMinDEEIntervals(int numMutable, int strandMut[][], PrunedRotamers<Boolean> prunedRotAtRes,
		...
		MinDEEIntervals compInt = new MinDEEIntervals(arpMatrix, arpMatrixMax, numMutable, 	strandMut, 
				strandRot, prunedRotAtRes, scaleInt, dist, maxIntScale, mutRes2Strand,mutRes2StrandMutIndex);
							...
							if (curLevel1!=curLevel2) {
								...
								/*if ((ligPresent)&&(curLevel2==numInAS)){ //the ligand level
									...
									for (int curAA2=0; curAA2<strandRot[str2].getNumAllowable(strResNum2); curAA2++){ //for all allowed AA's
										...
										int newRot2 = strandRot[str2].rl.getNumRotForAAtype(index2);
										if (newRot2==0)
											newRot2 = 1;
										for (int curRot2=0; curRot2<newRot2; curRot2++){ //for all rotamers for the given AA
	...
	public void DoBoundFlags(int numMutable, int strandMut[][],
		...
		BoundFlags bFlagsRun = new BoundFlags(arpMatrix,numMutable,strandMut,strandRot,
				pruningE,prunedRotAtRes,splitFlags,useSF,initEw, mutRes2Strand, mutRes2StrandMutIndex);
...
//////////////////////////////////////////////////////////////////////////
	...
	private float doAStarGMECHelper(int numMutable, int strandMut[][], String fileName, 
		...
		for (int curLevel=0; curLevel<treeLevels; curLevel++){
			...
			/*if ((ligPresent)&&(curLevel==(treeLevels-1))){ //the ligand level
				...
				for (int i=0; i<strandRot[str].getNumAllowable(strResNum); i++){ //add the rot for all allowable AA at this residue
					int newRot = strandRot[str].rl.getNumRotForAAtype(strandRot[str].getIndexOfNthAllowable(strResNum,i));
		...
		for (int curLevel=0; curLevel<treeLevels; curLevel++){ //for each residue
			...
			/*if ((ligPresent)&&(curLevel==(treeLevels-1))){ //the ligand level
				...
				for (int curAA=0; curAA<strandRot[str].getNumAllowable(strResNum); curAA++){ //for all allowed AA's
					...
					if (newRot==0)
						newRot = 1;			System.out.print(newRot+" Rot): ");
			...
			/*if ((ligPresent)&&(curLevel==(treeLevels-1))){ //the ligand level
				...
				for (int curAA=0; curAA<strandRot[str].getNumAllowable(strResNum); curAA++){ //for all allowed AA's
					...
					for (int curRot=0; curRot<newRot; curRot++){ //for all rotamers for the given AA
						...
						if (!eliminatedRotAtRes.get(curLevel,index,curRot)){ //not pruned, so add its index
							indicesEMatrixPos[curIndexRed] = curLevel;
							indicesEMatrixAA[curIndexRed] = index;
							indicesEMatrixRot[curIndexRed] = curRot;
							curIndexRed++;
						}
						eliminatedRotAtPosRed[pruningIndex] = eliminatedRotAtRes.get(curLevel,index,curRot);
						//logPS.println(pruningIndex+" "+curIndex+" "+eliminatedRotAtRes[curIndex]);logPS.flush();
						pruningIndex++;
					}
				}
		...
		for (int curRot1=0; curRot1<numTotalRotRedNonPruned; curRot1++){
			...
			for(int i=0; i<arpMatrix[p1][a1][r1][p1][0].length-1;i++)
				arpMatrixRed[numTotalRotRedNonPruned+i][curRot1] = arpMatrix[p1][a1][r1][p1][0][1+i];//store shell-rotamer E in the last rows
		}
		...
		while (true){
			...
			System.out.print("curRot: ");for(int i=0;i<numMutable;i++)System.out.print(curStrRotNum[i]+" ");//System.out.println(curLigRotNum);
	...
	private int getAAIndex(int rotIndex, int curRes, String strandDefault[][], int strandMut[][]){
		...
		for (int i=0; i<strandRot[str].getNumAllowable(strResNum); i++){
			int curRot = strandRot[str].rl.getNumRotamers(strandRot[str].rl.getAAName(strandRot[str].getIndexOfNthAllowable(strResNum,i)));
...
//////////////////////////////////////////////////////////////////////////////////
	...
	public static float getShellRotE(float Emat[][][][][][],int pos, int AANum, int rot){
		float shlRotE = 0.0f;
	...
	public static float getReducedShellRotE(float RedEmat[][],int index,int numTotalNodes){
		float shlRotE = 0.0f;
		//Skip the first energy which is the intra-rot energy still
