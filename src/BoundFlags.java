...
public class BoundFlags {
    ...
    //constructor
    BoundFlags(float arpMatrix[][][][][][], int numResMutable, int strMut[][], StrandRotamers strandLRot[], double pruneE, 
	...
	while (!done) {
		...
		for (int AA1=0; AA1<numAAtypes[curPos1]; AA1++){
		    ...
		    int numRotForCurAAatPos1 = strandRot[str1].rl.getNumRotForAAtype(curAA1);
			...
			if ((!eliminatedRotAtPos.get(curPos1,curAA1,curRot1))){ //not already pruned
				...
				for (int AA2=0; AA2<numAAtypes[curPos2]; AA2++){
					...
					if ((!eliminatedRotAtPos.get(index2))){ //not already pruned
						...
						curEc = RotamerSearch.getShellShellE(pairwiseMinEnergyMatrix);//pairwiseMinEnergyMatrix[pairwiseMinEnergyMatrix.length-1][0][0][0][0][0]; //initialize to Et' for each rotamer
	...
	minShellResE_ir = RotamerSearch.getShellRotE(pairwiseMinEnergyMatrix, posNum1, AANumAtPos1, rotNumAtPos1);//pairwiseMinEnergyMatrix[posNum1][AANumAtPos1][rotNumAtPos1][posNum1][0][1];

	minIndVoxelE_js = pairwiseMinEnergyMatrix[posNum2][AANumAtPos2][rotNumAtPos2][posNum2][0][0];	//formula term 1
	minShellResE_js = RotamerSearch.getShellRotE(pairwiseMinEnergyMatrix, posNum2, AANumAtPos2, rotNumAtPos2);//pairwiseMinEnergyMatrix[posNum2][AANumAtPos2][rotNumAtPos2][posNum2][0][1];
