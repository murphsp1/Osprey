...
/**
 * Manages the EEF1 solvation parameters; EEF1 handles only natural amino acids,
 * so solvation energies are computed for proteins and ligands (natural amino
 * acids) only, and not cofactors; Some important notes about assumptions of the
 * EEF1 model are given in the comments to getSolvationParameters()
 * 
 */
public class EEF1 implements Serializable {
    ...
    // These values are specific to eef1parm.dat, and may have to be modified
    // for different
	...
	// Group type Atom type Volume DGi(ref) DGi(free) DHi(ref) DCpi(ref)
	// lambda_i vdWri
	curLine = bufread.readLine();
    ...
    // The mapping is based on the group types in eef1parm.dat: different
    // versions
    // of the parameter file may require modifications in this function;
    // The assumption is made that the temperature in Kelvin is 298.15, since
    // this is
    // the temperature for which dGi(free) and dGi(ref) are derived in
    // eef1parm.dat
    // Note that some of the C and S groups are *extended* heavy atoms (heavy
    // atoms which include
    // the bonded hydrogen atoms), so in the original EEF1 and CHARMM
    // implementation, the
    // corresponding vdW radii are increased accordingly. We thus use the vdW
    // radii from the
    // extended Param 19 (see Appendix B, Neria & Karplus, 1996) parameters for
    // the solvation energy computation only;
    // The groups in eef1 are based on the natural amino acids. Additional
    // groups may have to be included,
    // and the current groups may have to be modified if new non-natural amino
    // acids or other
    // atom types or molecules (such as AMP or ATP) are included
    public boolean getSolvationParameters(int atom1, double dGref[],
	    double dGfree[], double atVolume[], double lambda[], double vdWr[]) {
    ...
    // This function assumes that the predfefined groups are as given in
    // eef1parm.dat;
    // Modifcations to this functions will be required if different group types
    // are used
    private int getSolvGroupIndex(Atom at1) {

	String elementType = at1.elementType;
	String AAname = m.residue[at1.moleculeResidueNumber].name; // the AA
								   // name to
								   // which this
								   // atom
								   // belongs
	...
	if (elementType.equalsIgnoreCase("C")) {
	    ...
	    if (at1.name.equalsIgnoreCase("CG")
		    && (AAname.equalsIgnoreCase("TYR")
			    || AAname.equalsIgnoreCase("PHE")
			    || AAname.equalsIgnoreCase("HIS")
			    || AAname.equalsIgnoreCase("HIP")
			    || AAname.equalsIgnoreCase("HID") || AAname
				.equalsIgnoreCase("HIE")))
		return findSolvGroup("CR"); // CG of (TYR or PHE or HIS)

	    else if ((at1.name.equalsIgnoreCase("CG")
		    || at1.name.equalsIgnoreCase("CD2") || at1.name
			.equalsIgnoreCase("CE2"))
		    && AAname.equalsIgnoreCase("TRP"))
		return findSolvGroup("CR"); // (CG or CD2 or CE2) of TRP

	    else if (at1.name.equalsIgnoreCase("CZ")
		    && AAname.equalsIgnoreCase("ARG"))
		return findSolvGroup("CR"); // CZ of PHE

	    else if ((aromatic) && (numBoundH == 1)) // extended aromatic C with
						     // 1 H, so CR1E group
		return findSolvGroup("CR1E");

	    else if ((!aromatic) && (numBoundH == 1)) // extended aliphatic C
						      // with 1 H, so CH1E group
		return findSolvGroup("CH1E");

	    else if ((!aromatic) && (numBoundH == 2)) // extended aliphatic C
						      // with 2 H, so CH2E group
		return findSolvGroup("CH2E");

	    else if ((!aromatic) && (numBoundH == 3)) // extended aliphatic C
						      // with 3 H, so CH3E group
	    ...
	    else if (AAname.equalsIgnoreCase("ARG") && (numBoundH == 2)) // guanidinium
									 // N in
									 // (side-chain
									 // of)
									 // ARG,
	    ...
	    else if ((aromatic) && (numBoundH == 0)) // aromatic (side-chain) N
						     // with 0 H, so NR group
		return findSolvGroup("NR");

	    else if (numBoundH == 2) // any (side-chain) N with 2 H, so NH2
				     // group
		...
		// all other oxygens (carbonyl oxygen)
    ...
    // Determines if ths given atom is a carboxyl oxygen (there should be only
    // one carbon bound to the
    // given oxygen and exactly two oxygens (including the given one) should be
    // bound to that carbon)
	...
	if (!at1.elementType.equalsIgnoreCase("O")) // not O
		...
		for (int i = 0; i < at1.bond.length; i++) {
		    ...
		    if (at2.elementType.equalsIgnoreCase("C")) { // found a
								 // bound C
	...
	boolean isHis = (AAname.equalsIgnoreCase("HIS")
		|| AAname.equalsIgnoreCase("HIP")
		|| AAname.equalsIgnoreCase("HID") || AAname
		.equalsIgnoreCase("HIE"));

	if (!(AAname.equalsIgnoreCase("PHE") || AAname.equalsIgnoreCase("TYR")
		|| AAname.equalsIgnoreCase("TRP") || isHis)) // not PHE and not
							     // TYR and not TRP
							     // or HIS, so not
							     // aromatic
	    return false;
		...
		if (at1.elementType.equalsIgnoreCase("N")
			&& (!at1.name.equalsIgnoreCase("N") && (AAname
				.equalsIgnoreCase("TRP") || isHis))) // N in the
								     // ring of
								     // TRP or
								     // HIS
		    return true;
		...
		if (atomName.equalsIgnoreCase("CA")
			...
			|| atomName.equalsIgnoreCase("C")) // CA or CB or C, so
							   // cannot be aromatic
		    return false;
		else
		    // aromatic
